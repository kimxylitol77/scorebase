...
- 유도 규칙(v2): 그룹 순위표로 팀→조 매핑 → 첫 크로스 그룹 매치부터 녹아웃 →
  **메인 사다리**(무패 팀끼리)와 **순위 결정전**(패자 낀 경기, 5~8위전 등)을 분리.
  라벨은 결승 식별 시 끝에서 역산(부전승 대회의 "24강" 오표 방지), 미식별 시 1라운드
  팀 수가 2^n 일 때만 N강. 3·4위전은 **결승 식별 후에만** 판정(안 그러면 8강 패자전 오표).
- **다단계 혼재 가드**: 한 팀이 두 조에 나타나면(예선+본선 캐시 혼재, VB_EURO_W 실측)
  유도 포기 — 본선 조별이 녹아웃으로 오판돼 엉터리 대진표가 선다. 엉터리보다 없음.
...
4. 감시 축 `tournament_bracket_gap`(전 종목) — 배구 재료 결손(vb_bracket_gap)과 **배구 외 종목의 새 조별 대회 출현**(IIHF 등 → 이 스킬로 연결하라는 경보)을 하루 2회 잡는다.
