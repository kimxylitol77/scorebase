...
//
//   코파 델 레이도 같은 구조(초반 라운드가 하부리그 팀)라 --league COPA_DEL_REY 로 같은 잡을 쓴다(2026-09-24).
//
//   실행: npx tsx --env-file=.env.local src/jobs/collect-fa-cup.ts [--league COPA_DEL_REY] [--future 14] [--past 3]
import "@/lib/env";
...
async function seedTeamMappings(league: League, matches: NormalizedMatch[]) {
  const tsIds = [
  ...
  for (const r of rows) {
    ...
    if (r.league === league || r.league === "CLUB_FRIENDLY") continue;
  ...
  for (const [externalId, teamId] of canonical) {
    await prisma.teamSourceId.upsert({
      where: { league_source_externalId: { league, source: "thesports", externalId } },
      create: { league, source: "thesports", externalId, teamId },
      update: {},
    });
  }
}
...
export async function collectFaCup(opts?: { futureDays?: number; pastDays?: number; league?: League }) {
  const league = opts?.league ?? "FA_CUP";
  const futureDays = opts?.futureDays ?? 14;
  const pastDays = opts?.pastDays ?? 3;
  const collector = buildTheSportsFootballCollector(league);
  const start = addDays(todayKST(), -pastDays);
  const end = addDays(todayKST(), futureDays);
  console.log(`[collect-fa-cup] ${league} ${start} ~ ${end} (TheSports diary)`);

  let total = 0;
  for (let d = start; d <= end; d = addDays(d, 1)) {
    try {
      const matches = await collector.fetchByDate(d);
      await seedTeamMappings(league, matches);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  collectFaCup({ futureDays: num("--future"), pastDays: num("--past"), league: i >= 0 ? (args[i + 1] as League) : undefined })
    .then(() => process.exit(0))
    .catch((e) => {
      console.error(e);
      process.exit(1);
