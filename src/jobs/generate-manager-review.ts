...
- 리서치 노트의 연속 기록(N경기 무패·연승 등)이나 특정 구간 서사가 제공된 월별 흐름과 어긋나면 쓰지 않는다.
  월별 승·무·패가 정본이다. 2026-08-19 실측: 4월·5월에 패가 있는 팀 글에 "3월 이후 13경기 무패" 가 실렸다.
- 출전 포메이션은 제공된 "포메이션 사용" 분포에 있는 것만 사실로 쓴다. 리서치 노트에 다른 포메이션이
  나와도 팀이 실제로 그 포메이션을 가동했다고 서술하지 말 것 — 글 상단 대시보드의 분포와 어긋난다.
  보유 시 유동 구조를 설명할 때는 라인업 표기가 아니라는 점이 문장에서 드러나게 쓴다.
...
async function main() {
  ...
  for (const teamId of targetIds) {
    ...
    // 대체문자(U+FFFD)가 섞이면 선수 이름이 통째로 깨진다 — 레버쿠젠 1차 생성에서 "프림퐁" 이
    // "프림포<FFFD>" 로 저장됐다(2026-08-19). 지면에 그대로 나가므로 저장하지 말고 버린다.
    if (content.includes("\uFFFD")) {
      console.warn(`[manager-review] 깨진 문자(U+FFFD) 포함 — skip: ${ctx.team.name}`);
      continue;
    }
