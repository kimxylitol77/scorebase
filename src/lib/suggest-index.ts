// 검색 자동완성 공용 — 한글 자모 분해·초성 매칭 + 축구 선수(PMV) 인메모리 인덱스 빌더.
// /api/transfers/suggest (이적시장 검색)과 /api/search/autocomplete (헤더 글로벌 검색)이 공유.
// 노출 규칙(18개월 활성·이름 미상 제외·동명+팀 dedup)은 /transfers 랭킹과 동일.
...
export const FIVE = ["EPL", "LALIGA", "BUNDESLIGA", "SERIE_A", "LIGUE_1"];
const FIVE_SET = new Set(FIVE);
// 팀명 미매핑 시 리그 라벨 폴백, 확장 리그는 팀명 옆 병기 (빅5는 클럽명으로 충분)
const PMV_LEAGUE_LABEL: Record<string, string> = {
...
export function krw(eurM: number): string {
...
export function decomp(s: string): string {
  let out = "";
  for (const ch of s) {
...
/** 초성 전용 쿼리면 공백 제거한 초성열, 아니면 null */
export function chosungQuery(qLower: string): string | null {
  return /^[ㄱ-ㅎ\s]+$/.test(qLower) ? qLower.replace(/\s+/g, "") : null;
}

export interface PlayerEntry {
  id: string; // ts player id → /transfers/{id}
  name: string;
  team: string; // 표시용 — "토트넘" / "LAFC · MLS" / 리그 라벨
  photo: string | null;
  value: string; // "1,523억" (1억 미만 "")
  v: number; // 정렬용 €
  nd: string; // 표시명 자모 분해 (lower)
  ndW: string[]; // 표시명 어절별 — "해리 케인"의 "케인" prefix 매칭
  ed: string; // 영문명 lower ("" = 없음)
  edW: string[];
  nc: string; // 표시명 초성
}

export interface Big5TeamAgg {
  ourId: number;
  name: string; // 한글 표시명
  logo: string | null;
  count: number;
...
/** PMV(시장가치) 전 리그 축구 선수 인덱스 + 빅5 팀 집계. 호출측에서 모듈 캐시할 것. */
export async function buildFootballPlayerIndex(): Promise<{ players: PlayerEntry[]; big5Teams: Big5TeamAgg[] }> {
  const cutoff = Math.floor(Date.now() / 1000) - 18 * 30 * 86400; // 18개월 활성 — 랭킹과 동일
  // history 는 마지막 항목의 market_time 만 필요 — JSON 전체 전송을 피해 SQL 에서 추출
  ...
  const teamAgg = new Map<number, Big5TeamAgg>();
  ...
  for (const r of raw) {
    ...
    dedup.add(k);
    const lgLabel = (r.league && PMV_LEAGUE_LABEL[r.league]) || "";
    ...
    players.push({
      ...
      value: eurM * 1e6 * EUR_KRW >= 1e8 ? krw(eurM) : "", // 1억 미만은 미표기
      v: r.currentValue || 0,
      nd: decomp(nameLower),
    ...
    if (tm && ourId != null && FIVE_SET.has(tm.league) && teamName) {
      const a = teamAgg.get(ourId) || { ourId, name: teamName, logo: tm.logoUrl, count: 0 };
      a.count++;
      teamAgg.set(ourId, a);
    }
  }
  return { players, big5Teams: [...teamAgg.values()] };
}

// 매칭 순위: 0 prefix(전체·어절·영문 어절 동급 — 동순위 내 가치순이 스타를 위로) > 1 이름 부분일치 > 2 영문 부분일치. -1 = 미일치
export function playerTier(e: PlayerEntry, qd: string, qc: string | null): number {
  if (qc) {
