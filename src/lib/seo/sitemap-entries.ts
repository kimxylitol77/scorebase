...
// sitemap 엔트리 빌더 — 두 벌을 한 번에 만든다.
//   lean = /sitemap.xml (구글·GSC 등록 주소): 허브·순위·예측·팀·감독·글 등 색인 가치 있는 URL 만
//   full = /sitemap-full.xml (빙 웹마스터에 별도 제출): live·h2h·선수 프로필 전량 포함(종전 sitemap.xml 과 동일 범위)
// 왜 가르는가(2026-09-03 GSC 실측): 12,439 URL 중 live·h2h·경량 transfers 등 구글용 noindex 페이지가
// 대부분이라 "제출 URL 대부분이 색인 금지"인 사이트맵이었고, 핵심 허브(/standings/EPL 등)는
// 아예 없어 구글이 "URL 모름". 빙은 live 페이지로 실제 유입이 있어 전체판을 유지한다.
//
...
export async function buildSitemapEntries(): Promise<{ lean: MetadataRoute.Sitemap; full: MetadataRoute.Sitemap }> {
  const base = SITE_URL;
  const now = new Date();
  ...
  const staticPages: MetadataRoute.Sitemap = [
    ...
    { url: `${base}/baseball/kbo-postseason`, changeFrequency: "daily", priority: 0.8 },
    { url: `${base}/baseball/npb-postseason`, changeFrequency: "daily", priority: 0.8 },
    ...(["MLB", "KBO", "NPB"] as const).map((lg) => ({ url: `${base}/baseball/postseason/stats?league=${lg}`, changeFrequency: "daily" as const, priority: 0.7 })),
  ...
  // 한국어 순위표 — 6/12 에 "순위 = 검색 정본 허브"로 정했는데 sitemap 엔 영어판만 있었다
  // (2026-09-03 GSC URL 검사: /standings/EPL·KBO "URL 모름"). 주요 리그 + 완료 시즌 아카이브.
  // 현재 시즌 라벨은 아카이브 라우트가 본 순위표로 307 하므로 제외(리디렉션 URL 등록 방지).
  const standingsLeagues = SITEMAP_LEAGUES.filter((lg) => STANDINGS_VALID.has(lg));
  const [archiveRows, currentLabels] = await Promise.all([
    prisma.seasonStandingsArchive.findMany({
      where: { league: { in: standingsLeagues } },
      select: { league: true, seasonLabel: true, updatedAt: true },
    }),
    Promise.all(
      standingsLeagues.map(async (lg) => {
        try {
          return [lg, seasonLabelFor(lg, await resolveSeasonYear(lg))] as const;
        } catch {
          return [lg, null] as const;
        }
      }),
    ),
  ]);
  const currentLabelOf = new Map(currentLabels);
  const standingsPages: MetadataRoute.Sitemap = [
    ...standingsLeagues.map((lg) => ({
      url: `${base}/standings/${lg}`,
      changeFrequency: "daily" as const,
      priority: 0.85,
    })),
    ...archiveRows
      .filter((r) => r.seasonLabel !== currentLabelOf.get(r.league))
      .map((r) => ({
        url: `${base}/standings/${r.league}/${r.seasonLabel}`,
        lastModified: r.updatedAt, // 시즌 종료 후 동결 — 마지막 upsert 시점
  ...
  const nationalTeamPagesKo: MetadataRoute.Sitemap = wcTeams.map((t) => ({
    url: `${base}/national-teams/${t.id}`,
    changeFrequency: "daily" as const,
    priority: 0.8,
  }));
  // 영어판(2026-08 en-mirror) — 허브에서 링크되는 48개국 상세(full 전용).
  // 감독 페이지(/en/coaches)는 경력 데이터가 한글 전용이라 섹션이 빠져 한국어판보다 얇아 제외.
  const nationalTeamPagesEn: MetadataRoute.Sitemap = wcTeams.map((t) => ({
  ...
  // 야구 선수는 리더보드 등재자만, UFC 파이터·영어판 선수·영어판 국가대표는 thin 이라 제외.
  const leanPlayerPages = playerPages.slice(0, 600);
  const leaderKeys = new Set(bbLeaders.filter((r) => r.externalId).map((r) => `${r.league}:${r.externalId}`));
  const leanBaseballPlayerPages = baseballPlayerPages.filter((p) => {
    const m = p.url.match(/\/players\/(\d+)(?:\?league=(\w+))?$/);
