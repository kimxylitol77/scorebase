// KBO 투수 시즌 스탯 리그 백분위 계산 — kbo-hitter-percentile 의 투수판.
// 데이터 소스는 BaseballPlayerSeasonStats 의 KBO 투수 row (PitcherBasic 스크랩, era not null).
...
async function compute(playerName: string, teamHint: string | null): Promise<KboPitcherPercentiles | null> {
  ...
  const rows: Row[] = await prisma.baseballPlayerSeasonStats.findMany({
    where: { league: "KBO", season: latest.season, era: { not: null }, ip: { not: null } },
    select: {
      playerName: true, teamName: true,
...
export const getKboPitcherPercentiles = unstable_cache(compute, ["kbo-pitcher-percentile"], {
  revalidate: 3600,
});
