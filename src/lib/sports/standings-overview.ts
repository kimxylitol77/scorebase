...
export async function safeFetchTop3(league: string): Promise<TopThreeEntry[]> {
  try {
...
/** 축구 — 국가별 그룹 (컵대회 제외). */
export async function fetchSoccerCountryGroups(): Promise<CountryStandingsGroup[]> {
  ...
  const fetched = await Promise.all(
    leagues.map(async (league) => ({ league, top3: await safeFetchTop3(league) })),
  );
...
export async function fetchSportGroups(detailPageCodes?: Set<string>): Promise<CountryStandingsGroup[]> {
  const groups: CountryStandingsGroup[] = [];
