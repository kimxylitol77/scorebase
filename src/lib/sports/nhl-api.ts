...
// DB Team.shortName 은 ESPN 약어라 네 팀이 NHL 공식 약어와 달라 roster 가 404 — 로스터 섹션이 통째로 빠졌다(2026-09-26 실측).
const ESPN_TO_NHL_ABBR: Record<string, string> = { LA: "LAK", NJ: "NJD", SJ: "SJS", TB: "TBL" };

export async function fetchNhlRoster(espnOrNhlAbbr: string, season: string): Promise<NhlRosterPlayer[]> {
  const abbr = ESPN_TO_NHL_ABBR[espnOrNhlAbbr.toUpperCase()] ?? espnOrNhlAbbr;
