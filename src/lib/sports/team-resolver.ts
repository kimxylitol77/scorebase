...
//      cross-league(3)보다 먼저 — 자기 리그에 진짜 row 가 있는데 컵 네임스페이스 row 를
//      물어오면 af/ts 팀 resolve 가 갈려 크로스소스 매치 중복이 생긴다
//      (2026-08-15 GEORGIA_EL·ESTONIA_ML 실측 — 도메스틱 매핑이 UECL row 를 가리킴).
//   3) cross-league lookup: 같은 (source, externalId) 가 다른 league 에 매핑되어 있고
...
//      Barcelona SC) 오매핑 방지.
//   4) 새 Team + TeamSourceId 동시 생성 (Team.externalId collision 시 기존 row 재사용)
...
export async function resolveTeamId(args: ResolveTeamArgs): Promise<number> {
  ...
  // 2) name fuzzy match (같은 league 안에서 normalize 일치) — cross-league 보다 먼저.
  if (norm) {
  ...
  // 3) cross-league lookup — 같은 (source, externalId) 가 다른 league 에 이미 매핑돼 있고
  ...
  // 4) 새 Team + 매핑 동시 생성. Team.externalId unique 충돌 시 기존 row 재사용.
  let teamId: number;
  try {
