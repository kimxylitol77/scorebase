// 야구 선수 페이지 공용 SEO 블록 — Person·BreadcrumbList JSON-LD + 데이터조립 소개 문단.
//   MLB/KBO/NPB 는 프로필 소스가 제각각(런타임 fetch)이라 각 View 가 정규화한 값을 넘긴다.
//   소개 문단은 위키 서사 복사가 아니라 보유 데이터 조립 — AI 검색 인용(GEO)용.
...
function buildAbout(p: BaseballSeoProps): string {
  const role = p.position ? p.position : "야구 선수";
  const parts = [
    `${p.name}${josa(p.name, "은", "는")} ${p.team ? `${p.team} 소속 ` : ""}${p.league} ${role}이다.`,
  ];
...
export default function BaseballPlayerSeo(props: BaseballSeoProps) {
  ...
  const personLd = athleteLd({
    ...
    jobTitle: `${props.league} ${props.position ?? "야구 선수"}`,
  ...
  return (
    <>
      ...
      <section className="text-sm leading-relaxed text-neutral-600 dark:text-neutral-300">
        <h2 className="sr-only">{props.name} 선수 소개</h2>
        <p>{about}</p>
      </section>
    </>
  );
