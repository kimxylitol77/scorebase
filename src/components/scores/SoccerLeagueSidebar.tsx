// 축구 페이지 좌측 사이드바 — 즐겨찾기·인기·오늘 경기 많은 리그 + 국가별 접힘 (AiScore 스타일).
// 서버는 "경기 있는 리그" 메타만 만들고, 구획 분배·즐겨찾기·접기·검색은 SoccerLeagueSidebarList(클라이언트).
...
export default function SoccerLeagueSidebar({
  ...
  const items: SidebarLeagueItem[] = leagues
    ...
    .map((l) => {
      const country = COUNTRY_BY_LEAGUE[l] ?? null;
      return {
        code: l,
        name: LEAGUE_DISPLAY[l] ?? l,
