// 축구 사이드바 리그 목록(클라이언트) — 즐겨찾기 → 인기 → 오늘 경기 많은 리그 → 국가별(기본 접힘).
// 네 구획은 서로 겹치지 않는다(같은 리그가 두 번 안 나옴). 검색창은 모든 리그를 평면으로 거른다.
// 즐겨찾기는 localStorage 라 SSR 은 빈 채로 나가고 hydration 후 한 번 채워진다(사이드바 한정 이동).
  ...
  return (
    <li className="relative">
      ...
      <button
        type="button"
        onClick={onToggleFav}
        aria-label={fav ? `${item.name} 즐겨찾기 해제` : `${item.name} 즐겨찾기 추가`}
        aria-pressed={fav}
    ...
    <div>
      ...
      {searched ? (
        <section className="mt-3">
          <SectionTitle>검색 결과 {searched.length}</SectionTitle>
          {searched.length === 0 ? (
            <p className="px-2 text-[11px] text-neutral-400">오늘 경기가 있는 리그 중 일치하는 항목이 없습니다.</p>
          ) : (
        ...
        <>
          {favItems.length > 0 && (
            <section className="mt-3">
              <SectionTitle>즐겨찾기 리그</SectionTitle>
          ...
          {popItems.length > 0 && (
            <section className="mt-4">
              <SectionTitle>인기 리그</SectionTitle>
          ...
          {todayItems.length > 0 && (
            <section className="mt-4">
              <SectionTitle>오늘 경기 많은 리그</SectionTitle>
          ...
          {countries.length > 0 && (
            <section className="mt-4">
              <SectionTitle>국가별 리그 {countries.length}</SectionTitle>
