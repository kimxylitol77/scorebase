// scores__golf__GolfLeaderboard (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
"use client";
      ...
      <table className="w-full text-center border-collapse whitespace-nowrap">
        ...
        <tbody>
          ...
          <tr className="text-[11px] text-neutral-500 bg-neutral-50 dark:bg-white/[0.02]">
            <td className="px-2 py-1 text-left font-semibold">Cumulative</td>
            {front.map((h) => (
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-neutral-200 dark:border-neutral-800">
      <table className="w-full text-center text-[12px]">
        <thead>
          <tr className="bg-neutral-100 dark:bg-white/[0.04] text-[11px] text-neutral-500 dark:text-neutral-400">
            <th className="px-3 py-1.5 text-left font-semibold">Round</th>
            <th className="px-3 py-1.5 font-semibold">Strokes</th>
...
function ExpandedRow({ leader }: { leader: GolfLeaderData }) {
  ...
  if (leader.rounds.length === 0) {
    return (
      <div className="px-4 py-3 text-[12px] text-neutral-400">No scores recorded yet.</div>
    );
  }
  ...
  if (holeRounds.length === 0) {
    return (
      <div className="px-3 sm:px-4 py-3 space-y-2 bg-neutral-50/60 dark:bg-white/[0.015]">
        ...
        <p className="text-[10px] text-neutral-400">This event does not provide hole-by-hole cards, so only round totals are shown. Source ESPN.</p>
      </div>
    );
  }
  ...
  return (
    <div className="px-3 sm:px-4 py-3 space-y-2.5 bg-neutral-50/60 dark:bg-white/[0.015]">
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-neutral-400">
        <span className="inline-flex items-center gap-1"><span className="inline-block w-3 h-3 rounded-full ring-[1.5px] ring-rose-500" />Eagle or better</span>
        <span className="inline-flex items-center gap-1"><span className="inline-block w-3 h-3 rounded-full ring-1 ring-rose-400" />Birdie</span>
        <span className="inline-flex items-center gap-1"><span className="inline-block w-3 h-3 rounded-[3px] ring-1 ring-sky-400" />Bogey</span>
        <span className="inline-flex items-center gap-1"><span className="inline-block w-3 h-3 rounded-[3px] ring-[1.5px] ring-sky-500" />Double bogey or worse</span>
      </div>
    </div>
  );
}
