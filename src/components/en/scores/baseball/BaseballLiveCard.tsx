// scores__baseball__BaseballLiveCard (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
...
import BaseballScore from "../../../scores/baseball/BaseballScore";
...
function parseInningFromLabel(
  ...
  const half = m[2] === "Top" ? "top" : m[2] === "Bot" ? "bottom" : null;
  return { inning, half };
}
...
export default function BaseballLiveCard(props: BaseballLiveCardProps) {
  ...
  const halfKo = half === "top" ? "Top" : half === "bottom" ? "Bot" : "";
  const inningText = baseballCtx?.isExtra ? "Extra" : inning ? `${inning}${halfKo}`.trim() : null;
    ...
    <div className="p-3 sm:p-4 space-y-3">
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          {doubleHeader && (
            <span
              ...
              title={`Doubleheader ${doubleHeader.index} of ${doubleHeader.total})`}
            >
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            type="button"
            title="See where the odds are moving"
            onClick={(e) => {
          ...
          <table className="text-[11px] sm:text-xs w-full sm:min-w-[360px]">
            <thead>
              <tr className="text-neutral-500 dark:text-neutral-500">
                <th className="text-left font-semibold py-1 pr-1.5 sm:pr-2 w-9 sm:w-10">Team</th>
                {inningIdx.map((i) => {
