// 축구 라이브 경기 행 (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.

"use client";

import { useEffect, useLayoutEffect, useRef, useState } from "react";
import { enMatchStatus } from "@/lib/i18n/en";
...
export default function SoccerLiveRow(props: SoccerLiveRowProps) {
  ...
  const scoreLink = (inner: React.ReactNode) => {
    ...
    return isExternal ? (
      <a href={href} target="_blank" rel="noopener noreferrer" className={cls} title="Match details">
        {inner}
      </a>
    ) : (
      <Link href={href} prefetch={false} className={cls} title="Match details">
        {inner}
      </Link>
    );
  };
  ...
  const statusNode = isLive ? (
    <span className="text-[12px] font-bold text-rose-500 tabular-nums whitespace-nowrap">
      {enMatchStatus(liveStatusLabel) || "LIVE"}
    </span>
  ) : isPostponed ? (
    <span className="text-[11px] text-neutral-500">PP</span>
  ) : isFinished ? (
    ...
    >
      ...
      {!hideLeague && (
        <button
          ...
          title={`${badge.label} league info (new window)`}
        >
          ...
          >
            ...
            {position != null ? (
              ...
              <button
                ...
                title={`FIFA rank ${fifaRank} — full rankings (new tab)`}
      ...
      <div data-scell="half" className="text-center text-[11px] tabular-nums whitespace-nowrap">
        {(isLive || isFinished) && soccerHalfScore ? (
          <span title="Half-time score" className="text-rose-600 dark:text-rose-400 font-semibold">
      ...
      <div data-scell="star" className="flex justify-center">
        <FavoriteStar
          ...
          meta={{
            ...
            awayScore,
            status,
            statusLabel: enMatchStatus(liveStatusLabel) ?? timeLabel,
            href: href ?? undefined,
          }}
        />
      </div>
      ...
      <div data-scell="info" className="flex items-center justify-start gap-1">
        {href && (
          <button
            ...
            title="AI model probabilities and key factors (new tab)"
            aria-label="AI analysis — model probabilities and key factors"
        ...
        {href && hasLineup && (
          <button
            type="button"
            onClick={openInNewTab(href)}
            title="Starting line-ups (new tab)"
...
function OddsCell({ odds }: { odds: MatchOdds | null }) {
  ...
  const aria = `Odds home ${f(odds.home)} draw ${f(odds.draw)} away ${f(odds.away)}${
    odds.over != null ? `, over/under ${odds.totalLine ?? ""} over ${f(odds.over)} under ${f(odds.under)}` : ""
  }${odds.books ? `, ${odds.books} books avg` : ""}${stale ? ", update delayed" : ""} — odds movement`;
  return (
    <div
      data-scell="odds"
      role="link"
    ...
    >
      ...
      <div className="flex gap-1 text-neutral-400 dark:text-neutral-500">
        <OddsPair label="H" value={f(odds.home)} trend={odds.trend?.home} />
        <OddsPair label="D" value={f(odds.draw)} trend={odds.trend?.draw} muted />
        <OddsPair label="A" value={f(odds.away)} trend={odds.trend?.away} />
      </div>
...
function OddsPair({
  ...
  return (
    <span className={`inline-flex items-baseline gap-px ${muted ? "opacity-70" : ""}`}>
      ...
      <span>{value}</span>
      {trend === 1 && <span className="text-[7px] text-rose-500" aria-label="up">▲</span>}
      {trend === -1 && <span className="text-[7px] text-blue-500" aria-label="down">▼</span>}
    </span>
  );
}
...
function useOddsAgo(updatedAt: number | null | undefined): string | null {
  ...
  useEffect(() => {
    ...
    setAgo(min < 1 ? "just now" : min < 60 ? `${min}m ago` : min < 1440 ? `${Math.floor(min / 60)}h ago` : `${Math.floor(min / 1440)}d ago`);
  }, [updatedAt]);
  return ago;
}
    ...
    >
      <div className="rounded-lg border border-neutral-200 dark:border-white/10 bg-white dark:bg-neutral-900 shadow-xl shadow-neutral-900/15 p-2.5 text-left min-w-[196px]">
        <div className="text-[10px] font-bold text-neutral-500 mb-1.5">
          Odds{odds.books ? ` · ${odds.books} books avg` : ""}
        </div>
        <div className="space-y-1 text-[11px]">
          <div className="flex justify-between">
            <span className="text-neutral-500">Home / Draw / Away</span>
          ...
          {odds.over != null && (
            <div className="flex justify-between pt-1 mt-1 border-t border-neutral-200 dark:border-white/10">
              <span className="text-neutral-500">Over/Under {odds.totalLine}</span>
              <span className="tabular-nums">O {f(odds.over)} / Under {f(odds.under)}</span>
            </div>
          )}
          {odds.hcHome != null && (
            <div className="flex justify-between">
              <span className="text-neutral-500">Handicap {odds.hcLine}</span>
          ...
          <div className="flex justify-between pt-1 mt-1 border-t border-neutral-200 dark:border-white/10 text-[10px] text-neutral-500">
            <span>Updated {ago ?? (odds.updatedAt == null ? "time unknown" : "")}</span>
            {stale ? (
              <span className="text-amber-600 dark:text-amber-400 font-semibold">update delayed</span>
            ) : odds.trend ? (
              <span>▲▼ vs opening</span>
            ) : null}
          </div>
          <div className="text-[9px] text-neutral-400 leading-snug">
            For reference only — not a betting recommendation.
          </div>
        </div>
      </div>
    </div>
...
function GoalsTooltip({
  ...
  const CardBadge = ({ kind }: { kind: "yellow" | "red" }) => (
    <span
      ...
      aria-label={kind === "yellow" ? "Yellow card" : "Red card"}
    />
  );
          ...
          <div className="space-y-1">
            ...
            {homeGoals.map((g, i) => (
              ...
              >
                ...
                <span className="shrink-0" aria-label="Goal">⚽</span>
                <span className="truncate">{g.player || "—"}</span>
                {g.penaltyKick && (
                  <span className="text-[9px] text-neutral-400 shrink-0">PK</span>
                )}
                {g.ownGoal && (
                  <span className="text-[9px] text-neutral-400 shrink-0">Own goal</span>
                )}
              </div>
            ))}
          </div>
...
export function SoccerLiveRowHeader() {
  return (
    ...
    >
      ...
      <div className="text-center px-2">Score</div>
      <div>Away</div>
      <div className="text-center">HT</div>
      <div className="text-center">Star</div>
      <div data-sinfo className="text-center">Info</div>
