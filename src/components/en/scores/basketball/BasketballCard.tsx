// scores__basketball__BasketballCard (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
...
export default function BasketballCard(props: BasketballCardProps) {
  ...
  const quarterText = quarter ? `${quarter}Q` : null;
        ...
        >
          {clock ? (
            ...
            <div className="text-xl font-black" style={{ color: "#22c55e" }}>
              {quarterText ?? liveStatusLabel ?? "Live"}
            </div>
          )}
        </div>
      )}
          ...
          <table className="text-[11px] sm:text-xs w-full table-fixed">
            <thead>
              <tr className="text-neutral-500">
                <th className="text-left font-semibold py-1 pr-1 w-9">Team</th>
                {periodIdx.map((i) => {
