// scores__FavoriteMatches (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
...
export default function FavoriteMatches({ matches }: Props) {
  ...
  function handleClearAll() {
    ...
    if (!confirm(`${ids.size} starred matches — clear them all?`)) return;
    clear();
  }
  ...
  return (
    <section className="space-y-3">
      ...
      <div className="flex items-center gap-2 px-1 flex-wrap">
        <h2 className="text-sm font-bold tracking-tight">⭐ My matches</h2>
        <span className="text-[11px] text-neutral-400 tabular-nums">
          {fav.length} matches · {sportOrder.length} sports
        </span>
        <a
          href="/account#favorites"
          className="text-[11px] font-semibold text-blue-600 dark:text-blue-400 hover:underline"
          title="My page — starred matches, teams and alerts"
        >
          Manage →
        </a>
        <div className="ml-auto flex items-center gap-1.5">
          ...
          <button
            ...
            title="Pin starred live matches (PiP) — drag to move"
          ...
          <ToggleBtn
            active={view === "compact"}
            onClick={toggleView}
            label={view === "compact" ? "Larger" : "Smaller"}
            icon={view === "compact" ? "large" : "compact"}
          />
          <ToggleBtn
            active={favSound}
            onClick={toggleFavSound}
            label={favSound ? "Sound on for starred" : "Starred sound"}
