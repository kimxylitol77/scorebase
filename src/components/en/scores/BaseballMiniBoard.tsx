// scores__BaseballMiniBoard (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
...
export default function BaseballMiniBoard({ ctx }: Props) {
  ...
  const half_ko = half === "top" ? "Top" : half === "bottom" ? "Bot" : "";
  const inningText = ctx.isExtra ? `Extra${inning ? ` ${inning > 9 ? inning : ""}` : ""}`.trim() : inning ? `${inning}${half_ko}` : "";
  const outsText = outs != null ? `${outs} out` : "";
