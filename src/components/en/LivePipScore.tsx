// LivePipScore (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
...
export default function LivePipScore() {
  ...
  async function openDocPip(opts?: { silent?: boolean }): Promise<boolean> {
    ...
    try {
      ...
      win.document.title = "My matches — Scorebase";
    ...
    <div className={`@container p-2 ${isDocMode ? "min-h-0 flex-1 overflow-y-auto" : "max-h-72 overflow-y-auto"}`}>
      {rows.length === 0 ? (
        <p className="px-2 py-6 text-center text-[12px] leading-relaxed text-neutral-500">
          No starred matches yet.
          <br />
          Tap the star on a match card.
        </p>
      ) : pipStyle === "broadcast" ? (
      ...
      <span className="ml-auto flex items-center">
        {!isDocMode && docPipSupported && (
          <button
            type="button"
            onClick={() => openDocPip()}
            aria-label="Pop out of the browser"
            title="Pop out into an always-on-top mini window"
        ...
        <button
          ...
          aria-label={pipStyle === "broadcast" ? "Compact view" : "Detailed view (logos, large score, match clock)"}
          title={pipStyle === "broadcast" ? "Compact view" : "Detailed view — logos, large score, match clock"}
        ...
        >
          {pipStyle === "broadcast" ? "Compact" : "Detailed"}
        </button>
        <button
          type="button"
          onClick={toggleSize}
          aria-label={size === "sm" ? "Larger" : "Smaller"}
          title={size === "sm" ? "Larger" : "Smaller"}
  ...
  return (
    <div
      ...
      role="dialog"
      aria-label="Starred live scores (PiP)"
    >
      {header(false)}
      {restoreArmed && (
        <p className="border-b border-rose-100 bg-rose-50/60 px-3 py-1.5 text-[10px] font-semibold text-rose-600 dark:border-rose-500/20 dark:bg-rose-500/10 dark:text-rose-400">
          Click anywhere to pop the mini window back out
        </p>
      )}
      {body(false)}
    </div>
