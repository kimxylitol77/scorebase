// LolStandingsTabs (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
"use client";
...
const TABS = ["Standings", "Players", "Champions", "Bans", "Team stats"];
const card =
  ...
  return (
    <div className="relative max-w-4xl mx-auto px-3 sm:px-6 py-6 sm:py-8 space-y-5">
      ...
      <nav className="flex items-center gap-2 text-xs text-neutral-500">
        ...
        <span>›</span>
        <span className="text-neutral-700 dark:text-neutral-300">{p.name} Standings & players</span>
      </nav>
      ...
      <header>
        ...
        <h1 className="mt-3 text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight break-keep">{p.name} Standings & players</h1>
        <p className="text-sm text-neutral-500 mt-2 break-keep">
          Korea · League of Legends Champions Korea · 2026 season · TheSports
        </p>
      </header>

      {/* 리그 스위처 — LCK ↔ LEC ↔ LCS ↔ LPL */}
      <div className="flex gap-1.5 flex-wrap">
        {([["LOL", "LCK"], ["LEC", "LEC"], ["LCS", "LCS"], ["LPL", "LPL"], ["EWC", "EWC"]] as const).map(([code, lbl]) => (
          <Link
            key={code}
            href={`/standings/${code}`}
          ...
          <table className="w-full text-sm">
            <thead className={head}>
              <tr>
                ...
                <th className={C + " w-12"}>W</th>
                <th className={C + " w-12"}>L</th>
                <th className={C + " w-16"}>Win %</th>
              </tr>
            </thead>
            <tbody>
              {p.standings.map((r) => {
        ...
        <div className={card}>
          <table className="w-full text-sm">
            <thead className={head}>
              <tr>
                <th className="py-2.5 px-3 text-left w-10">#</th>
                <th className="py-2.5 px-2 text-left">Players</th>
                <th className="py-2.5 px-2 text-left">Team</th>
                <th className={C + " w-16"}>KDA</th>
                <th className={C + " w-16"}>Win %</th>
                <th className={C + " w-16"}>CS/min</th>
              </tr>
            </thead>
            <tbody>
              {p.players.map((pl, i) => (
        ...
        <div className={card}>
          <table className="w-full text-sm">
            <thead className={head}>
              <tr>
                <th className="py-2.5 px-3 text-left w-10">#</th>
                <th className="py-2.5 px-2 text-left">Champions</th>
                <th className={C + " w-14"}>Picks</th>
                <th className={C + " w-16"}>Win %</th>
              </tr>
            </thead>
            <tbody>
        ...
        <div className={card}>
          <table className="w-full text-sm">
            <thead className={head}>
              <tr>
                <th className="py-2.5 px-2 text-left">Team</th>
                <th className={C + " w-16"}>Avg kills</th>
                <th className={C + " w-16"}>Dragons</th>
                <th className={C + " w-16"}>Towers</th>
                <th className={C + " w-16"}>Games</th>
              </tr>
            </thead>
            <tbody>
              {p.teams.map((t) => (
                <tr key={t.teamId} className="border-t border-neutral-100 dark:border-neutral-800">
                  ...
                  <td className={C + " tabular-nums text-neutral-500"}>{Math.round(t.avgMin)}min</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <p className="text-xs text-neutral-400 mt-3">Updated {p.updatedAt.slice(0, 10)}</p>
    </div>
  );
}
