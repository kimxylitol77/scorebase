// 오버/언더 차트·타일 (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
  ...
  return (
    <figure className="mt-4">
      <svg
        ...
        role="img"
        aria-label={`Distribution of over 2.5 rates across ${values.length} leagues.`}
      >
        {/* 기준 눈금 — 배경 역할이라 옅게 */}
        ...
        {buckets.map((c, i) => {
          ...
          return (
            <rect
              ...
              opacity={0.85}
            >
              <title>{`${mid - BUCKET / 2}–${mid + BUCKET / 2}% band — ${c} leagues`}</title>
            </rect>
          );
        })}
        {highlight != null ? (
      ...
      <figcaption className="mt-1 text-center text-[11px] text-neutral-500 dark:text-neutral-500">
        The horizontal axis is the over 2.5 rate; bar height is how many leagues fall in that band.
      </figcaption>
    </figure>
  );
}
