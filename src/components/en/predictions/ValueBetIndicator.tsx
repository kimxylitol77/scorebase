// predictions__ValueBetIndicator (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
...
export default function ValueBetIndicator({
  ...
  const labels: Array<"Home" | "Draw" | "Away"> = ["Home", "Draw", "Away"];
  ...
  return (
    <span
      ...
      title={`model ${(modelProbs[bestSide] * 100).toFixed(0)}% vs market ${(marketProbs[bestSide] * 100).toFixed(0)}% — +${(bestDelta * 100).toFixed(1)}%p Value`}
    >
