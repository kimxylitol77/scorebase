// LolForeignTabs (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
"use client";
...
const TABS = ["Standings", "Roster", "Players", "Champions"];
const POS_LABEL: Record<number, string> = { 1: "ADC", 2: "Mid", 3: "Top", 4: "Jungle", 5: "Support" };
const card =
    ...
    <div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className={headRow}>
                  ...
                  <th className="text-center py-2.5 px-3 font-semibold w-16">Win %</th>
                </tr>
              </thead>
              <tbody>
                {standings.map((r) => {
          ...
          {standings.map((r) => (
            <div key={r.teamId} className="rounded-2xl bg-white ring-1 ring-black/5 shadow-[0_24px_70px_-30px_rgba(15,23,30,0.18)] dark:bg-white/[0.04] dark:ring-white/10 dark:shadow-none p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs text-neutral-400 tabular-nums ml-auto">{r.win}W {r.lose}L</span>
              </div>
              {r.roster.length === 0 ? (
                <p className="text-xs text-neutral-400">No roster data</p>
              ) : (
      ...
      {tab === 2 && (
        <div className={card}>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className={headRow}>
                  <th className="text-left py-2.5 px-3 font-semibold w-10">#</th>
                  <th className="text-left py-2.5 px-2 font-semibold">Players</th>
                  <th className="text-center py-2.5 px-2 font-semibold w-16">KDA</th>
                  <th className="text-center py-2.5 px-2 font-semibold w-16">Win %</th>
                  <th className="text-center py-2.5 px-3 font-semibold w-16">CS/min</th>
                </tr>
              </thead>
              <tbody>
                {players.map((pl, i) => (
          ...
          <p className="text-[11px] text-neutral-400 px-4 py-2">5+ games played · sorted by KDA</p>
        </div>
      )}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className={headRow}>
                  <th className="text-left py-2.5 px-3 font-semibold w-10">#</th>
                  <th className="text-left py-2.5 px-2 font-semibold">Champions</th>
                  <th className="text-center py-2.5 px-2 font-semibold w-16">Picks</th>
                  <th className="text-center py-2.5 px-3 font-semibold w-16">Win %</th>
                </tr>
              </thead>
              <tbody>
