// HomeTodayMatches (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
"use client";
  ...
  return (
    <section className="max-w-6xl mx-auto px-4 sm:px-6 mt-5 sm:mt-6" aria-labelledby="today-matches-title">
      <div className="flex items-center justify-between mb-2.5">
        ...
        <Link href="/en/scores" className="text-xs font-medium text-blue-600 dark:text-blue-400 hover:underline">
          All matches →
        </Link>
      </div>
    ...
    >
      <div className="flex items-center justify-between text-[11px] text-neutral-500 mb-2">
        <span className="flex items-center gap-1 truncate">
          {fav && <Star className="h-3 w-3 text-amber-500" fill="currentColor" aria-label="My team" />}
...
function OddsLine({ m }: { m: HomeMatch }) {
  ...
  if (m.odds) {
    const o = m.odds;
    return (
      <div className={`mt-2 grid ${o.draw != null ? "grid-cols-3" : "grid-cols-2"} gap-1 text-[11px]`} aria-label="Odds">
        {cell("1", o.home.toFixed(2))}
        {o.draw != null && cell("X", o.draw.toFixed(2))}
        {cell("2", o.away.toFixed(2))}
      </div>
    );
  }
  if (m.prob) {
    ...
    return (
      <div className={`mt-2 grid ${p.draw != null ? "grid-cols-3" : "grid-cols-2"} gap-1 text-[11px]`} aria-label="Model probability">
        {cell("1", pct(p.home))}
        {p.draw != null && cell("X", pct(p.draw))}
        {cell("2", pct(p.away))}
      </div>
    );
  }
  return <div className="mt-2 text-[11px] text-neutral-400">Odds not yet available</div>;
}
