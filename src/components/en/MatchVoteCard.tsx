// MatchVoteCard (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
...
export default async function MatchVoteCard({ matchId }: { matchId: number }) {
  ...
  const homeName = toEnglishTeamName(match.homeTeam.name);
  const awayName = toEnglishTeamName(match.awayTeam.name);
  ...
  return (
    <section className="rounded-2xl border border-neutral-200/80 bg-white p-3.5 dark:border-white/10 dark:bg-white/[0.04]">
      <div className="mb-2 flex items-center justify-between">
        <h2 className="text-sm font-bold text-neutral-900 dark:text-white">Predictions</h2>
        <a href="/en/picks" className="text-[11px] text-rose-600 hover:underline dark:text-rose-400">
          My accuracy & rank →
        </a>
      </div>
      <MatchVoteButtons
