// MyTeamsStrip (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
"use client";
...
export default function MyTeamsStrip() {
  ...
  return (
    <div className="mb-4 flex items-center gap-2 overflow-x-auto py-1" aria-label="Jump to my teams">
      <span className="inline-flex shrink-0 items-center gap-1 text-[11px] font-bold uppercase tracking-[0.15em] text-amber-500">
        <Star className="h-3 w-3" fill="currentColor" aria-hidden /> My teams
      </span>
      {named.map((t) => (
        <Link
          key={t.id}
