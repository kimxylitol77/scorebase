// EwcStandings (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
...
export default async function EwcStandings({ name }: { name: string }) {
  ...
  const ensure = (t: { id: number; name: string; logoUrl: string | null }): TeamAgg => {
    let a = agg.get(t.id);
    if (!a) {
      a = { teamId: t.id, name: toEnglishTeamName(t.name), logo: t.logoUrl ?? "", seriesW: 0, seriesL: 0, setsW: 0, setsL: 0 };
      agg.set(t.id, a);
    }
    return a;
  };
  ...
  return (
    <div className="relative max-w-4xl mx-auto px-3 sm:px-6 py-6 sm:py-8 space-y-5">
      ...
      <nav className="flex items-center gap-2 text-xs text-neutral-500">
        ...
        <span>›</span>
        <span className="text-neutral-700 dark:text-neutral-300">{name} Group standings</span>
      </nav>

      <header>
        <span className="inline-flex items-center gap-1.5 rounded-full bg-rose-500/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.15em] text-rose-600 ring-1 ring-rose-500/20 dark:text-rose-400">
          <span className="h-1.5 w-1.5 rounded-full bg-rose-500" aria-hidden /> Esports World Cup
        </span>
        <h1 className="mt-3 text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight break-keep">{name} LoL group standings</h1>
        <p className="text-sm text-neutral-500 mt-2 break-keep">
          League of Legends · group stage (BO5) · 2026 · TheSports
        </p>
      </header>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className={headRow}>
                  ...
                  <th className="text-center py-2.5 px-2 font-semibold w-12">W</th>
                  <th className="text-center py-2.5 px-2 font-semibold w-12">L</th>
                  <th className="text-center py-2.5 px-3 font-semibold w-20">Sets</th>
                </tr>
              </thead>
              <tbody>
                {standings.map((r, i) => (
            ...
            {finished.map((m) => {
              ...
              return (
                <li key={m.id} className="flex items-center gap-3 px-4 py-3 text-sm">
                  ...
                  <span className={`flex-1 text-right truncate ${homeWin ? "font-bold" : "text-neutral-500"}`}>{toEnglishTeamName(m.homeTeam.name)}</span>
                  <span className="tabular-nums font-black px-2 shrink-0">{m.homeScore} : {m.awayScore}</span>
                  <span className={`flex-1 truncate ${!homeWin ? "font-bold" : "text-neutral-500"}`}>{toEnglishTeamName(m.awayTeam.name)}</span>
                </li>
              );
            })}
          </ul>
          ...
          <ul className="divide-y divide-neutral-100 dark:divide-white/5">
            {upcoming.map((m) => (
              <li key={m.id} className="flex items-center gap-3 px-4 py-3 text-sm">
                ...
                <span className="flex-1 text-right truncate font-semibold">{toEnglishTeamName(m.homeTeam.name)}</span>
                <span className="text-neutral-400 px-2 shrink-0">vs</span>
                <span className="flex-1 truncate font-semibold">{toEnglishTeamName(m.awayTeam.name)}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <p className="text-[11px] text-neutral-400 text-center pt-1">
        ⓘ The Esports World Cup runs a group stage followed by knockouts · the bracket appears once confirmed · times in KST
      </p>
    </div>
  );
}
