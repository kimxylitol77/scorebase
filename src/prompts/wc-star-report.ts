...
export function buildStarReportPrompt(d: StarReportData): string {
  ...
  L.push("- '슛 결정력'은 골/슛(득점 전환율)이며, '유효슛 비율'(유효슛/슛)과 다른 지표다 — 둘을 섞어 계산하지 말 것. 효율 지표의 '슛 결정력 %'는 대회 누적값이다. 경기 단위 결정력을 이 대회 누적 %와 임의로 동일시하지 말 것(예: 이 경기 4슛 2유효를 근거로 '28% 결정력'이라 적는 것은 오류). 경기 단위 수치는 이 경기 골/슛/유효슛 그대로만 서술.");
  L.push("- 효율 지표는 제시된 계산값의 의미만 서술. 경기 내 시점(초반·후반 득점 등)·시간에 따른 추세(점점 나아진다·상승세)·컨디션 변화는 경기별 데이터가 없으니 추론·단정 금지.");
  L.push("- 자연스러운 한국어로 쓸 것. 영어 직역투(예: '생물학적 기반'·'생물학적 정점')는 피하고 '신체적 전성기' 같은 한국 스포츠 저널리즘 어휘를 쓴다.");
