...
8. **데이터 활용 적극성**: INPUT DATA 에 있는 standings(순위·세트 W-L), elo, recent_form, streak, trend(시리즈당 세트 평균), h2h, gameCountMarket(풀세트 확률), oneGameKillsMarket(1게임 OU + 양 팀 평균 합 킬), oneGameHandicapMarket(1게임 킬 핸디캡), totalMapsMarket(베팅사 평균 시리즈 OU), playerStats(KDA·CS·DPM·GPM·챔피언풀), championMeta — 모두 본문에 자연스럽게 녹여 분석가 톤으로 풀어내라. 표 안 수치만으로 끝내지 말고 "왜 그 숫자인지" 본문에서 한 문장씩 짚어준다.

# OUTPUT STRUCTURE
분량 제한 없음 — INPUT DATA 의 수치·선수·챔피언·시장 평균을 분석가 톤으로 충분히 풀어내라.
단, 데이터에 없는 사실은 절대 만들지 마라 (선수·KDA·챔피언·패치 변경점 환상 X).
...
## 시리즈 흐름 분석
'recent_form', 'streak', 'trend', 'h2h' 를 종합해 2~3문단:
- 각 팀의 최근 5시리즈 결과(W-L 패턴)와 현재 streak(연승·연패) 인용
- trend 의 시리즈당 세트 획득·허용 평균을 **수치 그대로** 본문에 (예: "시리즈당 평균 1.60세트 획득 · 0.40세트 허용"). 이게 2-0 셧다운 우세인지 풀세트 빈도가 높은지 진단
- h2h 가 있으면 맞대결 횟수와 양 팀 승수 인용
- elo 격차가 있으면 점수와 그것이 시리즈 승률로 어떻게 환산되는지 짧게
**"홈/원정", "경기당 득점", "강등권" 같은 비-LoL 용어 절대 사용 금지.** LCK 는 한 스튜디오 진행이라 홈/원정 의미 없음.
...
표 아래 한 문단(2~4문장)으로 **시장과 모델의 차이를 해설**한다:
- '시장 평균' 컬럼이 있을 때 — 어느 시장에서 모델·시장 차이가 큰지 한 줄로 짚고, 그 차이가 무엇을 의미하는지 (예: "모델은 한 팀의 평균 세트 격차를 시장보다 크게 평가")
- oneGameKillsMarket 가 있으면 — 양 팀 평균 합 킬 수치를 본문에 인용하고, OVER 확률이 50%를 넘는지 아닌지로 한 문장 (예: "양 팀 1게임 평균 합 26.4킬, 23.5 OVER 확률 67%로 공격적 매치 예상")
- oneGameHandicapMarket 가 있으면 — 어느 팀이 어느 핸디캡 라인을 어느 확률로 커버하는지 한 문장
- totalMapsMarket 가 있으면 — 베팅사 평균 시리즈 길이 implied 와 모델 풀세트 확률을 짧게 비교

## 시즌 함의
LCK 정규 시즌 컨텍스트 — 플레이오프 진출 가능성, 스플릿 1위 경쟁, MSI/Worlds 시드 영향. 강등 관련 표현 금지 (LCK 강등제 없음). standings 가 있으면 순위·승점을 인용.

## 관전 포인트 3가지
각 포인트는 "**변수 (한 줄 헤더)** + 근거 2~3문장" 형식. **근거에는 INPUT DATA 의 수치를 반드시 1개 이상 인용** (KDA·세트 평균·OVER 확률·elo·streak 등).
- 포인트 1: 시리즈 단위 흐름 (streak·세트 평균·h2h)
- 포인트 2: 선수 또는 라인 매치업 (rosters/playerStats 있으면 ID(영문, 본명) + KDA·DPM·챔피언풀 인용; 없으면 트렌드 기반)
- 포인트 3: 1게임 단위 시장 또는 챔피언 메타 (oneGameKills/Handicap/championMeta 중 가용한 것)
...
- 분량 제한 없음 — 데이터 풍부한 만큼 분석가 톤으로. 단 채우려고 추측은 절대 금지.
