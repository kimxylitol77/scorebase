...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // PREVIEW + RECAP(AI 자동 발행) 색인 정책 — 시간창 선별 (2026-07-17).
  //
  // 배경: 2026-05-21 구글 노출이 주 8,024 → 196 (-97.6%) 붕괴 후 6주째 주 8 로 평평.
  // 수동 조치는 없음(=알고리즘). 붕괴 대응으로 5/27(596297c) 자동글 2,620개를 전부
  // noindex 했는데, GSC 실측상 **붕괴 전 트래픽의 대부분이 바로 그 자동글**이었다
  // (mlb-preview-1692 단독 1,211 노출, "브루어스 대 트윈스" 1,195). 즉 전면 noindex 가
  // 회복 경로까지 잠갔음. 반면 전면 해제는 붕괴를 부른 '대량' 신호를 그대로 재현한다.
  //
  // 절충: **경기 시점 기준 D-7 ~ D+2 만 색인**. 근거 —
  //   · 검색 수요("{팀} 대 {팀}")는 경기 전후에 몰린다 → 가치의 대부분을 회수
  //   · 동시 색인 ~110~200개(2,620 대비 96%↓) → '대량 자동생성' 신호 소멸
  //   · 철 지난 프리뷰는 실제로 얼린 콘텐츠 → noindex 가 정직한 신호
  //   · 창이 지나면 자연히 빠져 색인 규모가 스스로 유지된다
  // startTime 이 없으면(매치 미연결) 보수적으로 noindex.
  const AUTO_INDEX_BEFORE_MS = 7 * 24 * 3600 * 1000; // 경기 7일 전부터
  const AUTO_INDEX_AFTER_MS = 2 * 24 * 3600 * 1000; // 경기 2일 후까지
  const isAutoGenerated = article.type === "PREVIEW" || article.type === "RECAP";
  const kickoff = article.match?.startTime?.getTime();
  const now = Date.now();
  const inIndexWindow =
    kickoff != null &&
    now >= kickoff - AUTO_INDEX_BEFORE_MS &&
    now <= kickoff + AUTO_INDEX_AFTER_MS;
  const autoNoIndex = isAutoGenerated && !inIndexWindow;
  return {
    title: displayTitle,
    description: desc,
    ...(keywords ? { keywords } : {}),
