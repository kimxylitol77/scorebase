...
import { buildBaseballPlayerHrefs, withNpbExternalIds } from "@/lib/sports/baseball-player-link";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: finished ? `${away} vs ${home} 결과·스코어 — NPB` : `${away} vs ${home} 라이브 — NPB`,
    description: finished
      ? `${away} vs ${home} NPB 일본프로야구 경기 결과 — 최종 스코어와 이닝별 점수 · 안타·실책 · 양팀 선발투수.`
      : `${away} vs ${home} NPB 일본프로야구 라이브 스코어 · 이닝별 점수 · 안타·실책 · 양팀 선발투수.`,
...
export default async function NpbLivePage({ params }: Props) {
  ...
  const rosterHrefs = buildBaseballPlayerHrefs({ league: "NPB", teamIds: [match.homeTeam.id, match.awayTeam.id], playerNameById });
  ...
  const venueCity = getBaseballCity("NPB", match.homeTeam.name);
          ...
          {seasonAnalysis?.hasData && (
            <>
              ...
              <BaseballBatterStats
                ...
                homeNameKo={homeKo}
                awayNameKo={awayKo}
                homeBatters={withNpbExternalIds(seasonAnalysis.home.batters, match.homeTeam.id)}
                awayBatters={withNpbExternalIds(seasonAnalysis.away.batters, match.awayTeam.id)}
              />
            </>
          )}
          {recentGames?.hasData && (
