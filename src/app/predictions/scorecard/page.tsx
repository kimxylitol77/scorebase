        ...
        {leader && (
          <dl className="mt-6 grid grid-cols-2 gap-2 sm:grid-cols-4">
            {[
              ...
              { k: "채점 누적", v: totalGraded.toLocaleString(), sub: `AI ${board.length}개 · 3시장 합산` },
              {
                k: "어제 합산",
                v: ydayAll.graded > 0 ? `${(ydayAll.rate * 100).toFixed(1)}%` : "—",
                sub: ydayAll.graded > 0 ? `${ydayAll.correct}/${ydayAll.graded} 픽 적중` : "채점 대기",
              },
              {
                k: "최근 7일 합산",
                v: r7All.graded > 0 ? `${(r7All.rate * 100).toFixed(1)}%` : "—",
                sub: r7All.graded > 0 ? `${r7All.correct}/${r7All.graded} 픽 적중` : "채점 대기",
              },
            ].map((s) => (
      ...
      <section className="mb-12">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 flex items-center gap-2 text-[11px] tabular-nums text-zinc-400 dark:text-white/30">
                      <span>{b.tally.correct} / {b.tally.graded} 적중</span>
                      {b.r7.graded >= RANK_MIN && (() => {
                        ...
                        return (
                          <span
                            ...
                            title="최근 7일 적중률 − 누적 적중률"
                          >
                            7일 {up ? "▲" : "▼"} {Math.abs(delta * 100).toFixed(1)}%p
                          </span>
                        );
                      })()}
                    </div>
                ...
                <div className="mt-3 grid grid-cols-5 gap-1 border-t border-black/5 pt-2.5 dark:border-white/5">
                  {[
                    { k: `최근 ${RECENT_WINDOW}건`, t: { ...b.recent, predicted: b.recent.graded } as Tally },
                    { k: "어제", t: b.yday },
                    { k: "7일", t: b.r7 },
                    { k: "14일", t: b.r14 },
                    { k: "30일", t: b.r30 },
                    ...
                    <div key={s.k} className="min-w-0 text-center">
                      ...
                      <div className="text-[10px] tabular-nums text-zinc-400 dark:text-white/30">
                        {s.t.graded > 0 ? `${s.t.correct}/${s.t.graded}` : "채점 없음"}
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {tiedTop >= 2 && (
          <p className="mt-3 text-center text-[13px] text-zinc-500 dark:text-white/40">
            1위~{tiedTop}위 격차 <strong className="text-zinc-700 dark:text-white/70">{(topGap * 100).toFixed(1)}%p</strong> —
            수천 건 표본에서도 오차 범위 안이라 사실상 동률입니다. 순위보다 기간별 추세와 시장·리그별 강약을 보세요.
          </p>
        )}
        ...
        <section className="mb-12">
          ...
          {(() => {
            const renderCard = (e: UpMatch) => {
              ...
              // 만장일치는 **패널 전원이 픽을 냈고 전원이 같은 편**일 때만 (2026-09-12).
              // 예전엔 픽을 낸 AI 끼리만 같으면 "만장일치 3" 이라 찍어, 7개 중 3개만 낸 경기가
              // 7개 전원 합의와 같은 배지를 달았다. 분모는 두 배지 모두 패널 수로 통일한다.
              const unanimous = !split && con.total >= aiModels.length;
              return (
                <div key={e.matchId} className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-zinc-200/70 dark:bg-white/[0.04] dark:ring-white/10">
                  <div className="flex items-center gap-2 text-xs text-zinc-500 dark:text-white/40">
                    ...
                    <span className={`ml-auto rounded-full px-2 py-0.5 text-[10px] font-bold ${split ? "bg-amber-500/10 text-amber-700 dark:text-amber-300" : unanimous ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300" : "bg-zinc-500/10 text-zinc-600 dark:text-white/50"}`}>
                      {split
                        ? `의견 갈림 ${con.agree}/${aiModels.length}`
                        : unanimous
                          ? `만장일치 ${aiModels.length}`
                          : `AI ${con.agree}/${aiModels.length}`}
                    </span>
                  </div>
                  ...
                  <div className="mt-2.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-[12px] text-zinc-500 dark:text-white/45">
                    ...
                    <span className="tabular-nums">AI {con.agree}/{aiModels.length} 동의 · 평균 확신 {(con.avgProb * 100).toFixed(0)}%</span>
                  </div>
