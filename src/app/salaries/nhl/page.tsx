...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: "https://www.scorebase.kr/salaries/nhl",
...
export default async function NhlSalariesPage({ searchParams }: Props) {
  ...
  if (view === "team") {
    ...
    return (
      <main className="relative max-w-3xl lg:max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-14 space-y-6">
        <AmbientGlow />
        <PlayerValueTabs active="/salaries/nhl" />
        ...
        <div className="overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-[0_24px_70px_-30px_rgba(15,23,30,0.18)] dark:border-neutral-800 dark:bg-white/[0.04] dark:shadow-none">
          <table className="w-full text-sm">
            ...
            <tbody>
              {teamRows.map((t, i) => {
                const top3 = i < 3;
                const ko = toKoreanTeamName(t.name, "NHL") ?? t.name;
                ...
                return (
                  <tr key={t.name} className="border-b border-neutral-100 dark:border-neutral-800/60 last:border-0 transition-colors duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] hover:bg-neutral-50 dark:hover:bg-white/[0.04]">
                    ...
                    <td className="px-2 py-2.5">
                      <Link href={`/salaries/nhl?team=${encodeURIComponent(t.name)}`} className="flex items-center gap-2.5 hover:underline">
                        {logo && (
  ...
  const teamKo = teamParam ? (toKoreanTeamName(teamParam, "NHL") ?? teamParam) : null;
        ...
        subtitle={
          q
            ...
            : teamKo
              ...
              : `선수별 연봉(cap hit) 순위 (달러·원화) · 전체 ${total.toLocaleString()}명 · 매주 자동 갱신`
        }
      />
      <ViewToggle view="player" teamLabel={teamKo} />

      {/* 선수검색 — GET 폼(?q=), 영문·한글명 모두 매칭 */}
      <form method="get" action="/salaries/nhl" className="flex gap-2">
        <input
          type="search"
          name="q"
          defaultValue={q}
            ...
            <nav className="flex items-center justify-center gap-1 text-sm">
              ...
              {pageList(page, totalPages).map((p, i) =>
                p === "…" ? (
                  ...
                  <Link
                    key={p}
                    href={p === 1 ? "/salaries/nhl" : `/salaries/nhl?page=${p}`}
  ...
  return (
    <header className="space-y-2">
      <div className="flex items-center gap-2 text-xs font-medium text-neutral-400">
        <Link href="/hockey" className="hover:underline">하키</Link>
        <span>›</span>
...
function ViewToggle({ view, teamLabel }: { view: "player" | "team"; teamLabel?: string | null }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <Link href="/salaries/nhl" className={pill(playerOn)}>선수별</Link>
      <Link href="/salaries/nhl?view=team" className={pill(view === "team")}>팀별</Link>
      {teamLabel && (
        <Link
          href="/salaries/nhl"
...
function PageLink({ page, disabled, label }: { page: number; disabled: boolean; label: string }) {
  ...
  return (
    <Link
      href={page === 1 ? "/salaries/nhl" : `/salaries/nhl?page=${page}`}
