...
export const metadata: Metadata = {
  title: "NBA 연봉 순위 — 2025-26 선수·팀별 연봉 (한화)",
  description:
...
export default async function NbaSalariesPage({ searchParams }: Props) {
      ...
      all
        ...
        .filter(
          (n) =>
            ...
            toKoreanPlayerName(n).includes(q) ||
            (lookupNbaPlayer(n)?.ko ?? "").includes(q),
        ),
    )];
  }
  ...
  return (
    <main className="relative max-w-3xl lg:max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-14 space-y-6">
      ...
      <form method="get" action="/salaries/nba" className="flex gap-2">
        <input
          type="search"
          name="q"
          defaultValue={q}
