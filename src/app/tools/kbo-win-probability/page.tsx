// KBO 승리확률 계산기 — 상황별 WE + 전술 손익 인터랙티브 도구 (국내 최초 KBO 전용).
...
export const metadata: Metadata = {
  title: "KBO 승리확률 계산기 — 상황별 승률·전술 손익 | 스코어베이스",
  description:
    "이닝·점수차·아웃·주자만 입력하면 KBO 경기 상황별 승리확률(Win Expectancy)을 즉시 계산합니다. 번트·도루·고의4구 등 전술이 승률을 얼마나 바꾸는지까지 보여주는 국내 최초 KBO 전용 전략 계산기.",
  keywords: [
    "KBO 승리확률", "KBO 승률 계산기", "야구 승리확률", "야구 상황별 승률",
    "Win Expectancy", "야구 전술 계산기", "번트 승률", "KBO 전략",
  ],
  alternates: { canonical: `${SITE_URL}/tools/kbo-win-probability` },
  openGraph: {
    title: "KBO 승리확률 계산기 — 상황별 승률·전술 손익",
    description: "이닝·점수차·아웃·주자로 KBO 승리확률을 즉시 계산. 번트·도루의 승률 손익까지.",
    url: `${SITE_URL}/tools/kbo-win-probability`,
    type: "website",
  },
};
...
const FAQ = [
  ...
  {
    q: "숫자는 어떻게 계산되나요?",
    a: "KBO 리그 평균 타격 환경으로 마르코프 모델과 몬테카를로 시뮬레이션을 돌려 모든 상황의 승리확률을 미리 계산했습니다. 리그 평균 기준이라 특정 투수·타자·구장은 반영하지 않으며, 방향성과 상대적 크기를 읽는 용도로 적합합니다.",
  },
  {
    q: "MLB·NPB도 되나요?",
    a: "같은 엔진을 리그별 득점 환경으로 보정해 확장할 예정입니다. KBO를 시작으로 메이저리그·일본프로야구 버전도 순차 공개합니다.",
  },
];

export default function KboWinProbabilityPage() {
  const jsonLd = [
    {
      ...
      url: `${SITE_URL}/tools/kbo-win-probability`,
      description:
        "KBO 경기 상황별 승리확률과 번트·도루·고의4구 등 전술의 승률 손익을 계산하는 인터랙티브 도구.",
  ...
  return (
    <main className="relative mx-auto max-w-3xl px-4 sm:px-6 py-10">
      ...
      <nav className="mb-3 flex items-center gap-1.5 text-xs text-zinc-500 dark:text-white/40">
        ...
        <span>도구</span>
        <span>›</span>
        <span className="text-rose-600 dark:text-rose-400">KBO 승리확률 계산기</span>
      </nav>
      ...
      <header className="mb-6">
        ...
        <p className="mt-3 text-[15px] leading-relaxed text-zinc-600 dark:text-white/60 break-keep">
          이닝·점수차·아웃·주자를 입력하면 KBO 경기의 <strong>상황별 승리확률</strong>을 즉시 계산합니다.
          번트·도루·고의4구가 승률을 얼마나 바꾸는지까지 보여주는 국내 최초 KBO 전용 전략 계산기입니다.
        </p>
      </header>

      <WinProbabilityTool table={table} leagueLabel="KBO" />
      ...
      <section className="mt-12 space-y-8 text-[15px] leading-relaxed text-zinc-700 dark:text-white/70">
        ...
        <div>
          ...
          <p className="break-keep">
            ...
            공격팀 승률을 낮추는 선택입니다. 한국 야구의 번트 통념을 숫자로 검증해 볼 수 있습니다.
          </p>
        </div>
        <div>
        ...
        <p className="text-[13px] text-zinc-500 dark:text-white/40 break-keep">
          KBO 경기 일정·순위는{" "}
          <Link href="/leagues/KBO" className="text-rose-600 underline-offset-2 hover:underline dark:text-rose-400">KBO 리그</Link>
          ,{" "}
          <Link href="/standings/KBO" className="text-rose-600 underline-offset-2 hover:underline dark:text-rose-400">KBO 순위</Link>
          에서, 우리 AI의 KBO 승부예측 적중률은{" "}
