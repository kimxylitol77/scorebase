"use client";
// 라인업 피치 — 좌표 절대배치 + 드래그(vanilla Pointer Events). 단일/맞대결 양 팀 렌더.
// 6px 이동 임계로 클릭(후보 열기)과 드래그(이동) 구분. setPointerCapture로 경계 밖도 추적.
import { useRef, useCallback } from "react";
import { Plus } from "lucide-react";
import type { Side } from "@/lib/lineup/lineup-state";
import { SIDE_COLORS } from "@/lib/lineup/formations";
...
export default function Pitch({ home, away, mode, poolById, kitFrom, kitTo, activeUid, onNodeClick, onNodeMove }: Props) {
  ...
  const onMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      const px = Math.max(4, Math.min(96, ((e.clientX - r.left) / r.width) * 100));
      const py = clampY(((e.clientY - r.top) / r.height) * 100, d.side);
      onNodeMove(d.side, d.uid, Math.round(px), Math.round(py));
    },
    [clampY, onNodeMove],
  );

  const onUp = useCallback(
    (e: React.PointerEvent, uid: string, side: "home" | "away") => {
      const d = dragRef.current;
  ...
  return (
    <div
      ...
      style={{ aspectRatio: "4 / 5", background: `linear-gradient(to bottom, ${kitFrom}, ${kitTo})` }}
    >
        ...
        side.players.map((pl) => {
          ...
          return (
            <button
              ...
              style={{ left: `${pl.x}%`, top: `${pl.y}%`, transform: "translate(-50%, -50%)", width: "21%" }}
            >
                ...
                {player ? (
                  <span className="relative block h-[clamp(34px,8vw,56px)] w-[clamp(34px,8vw,56px)] overflow-hidden rounded-full bg-white/90" style={{ boxShadow: `0 0 0 2px ${ring}` }}>
                    {player.photo ? (
                      ...
                      <img src={player.photo} alt={player.name} className="h-full w-full object-cover" draggable={false} />
                    ) : (
