    ...
    <div className="min-h-screen bg-white dark:bg-neutral-950 text-neutral-900 dark:text-white py-8 px-4">
      <div className="max-w-md mx-auto">
        ...
        {bench && bench.length > 0 && (
          <div className="mt-5">
            <div className="text-xs font-bold text-neutral-600 dark:text-neutral-400 mb-2">후보 선수 · 차순위</div>
                ...
                const inner = (
                  <>
                    <div className="w-7 h-7 rounded-full overflow-hidden bg-neutral-200 dark:bg-neutral-800 shrink-0 flex items-center justify-center ring-1 ring-black/10 dark:ring-white/15">
                      ...
                      ) : (<span className="text-[10px] font-bold text-neutral-500 dark:text-neutral-400">{koOf(p).slice(0, 1)}</span>)}
                    </div>
                    <div className="min-w-0 leading-tight">
                      <div className="text-[10px] font-bold truncate">{p.flag} {koOf(p)}</div>
                      <div className="text-[9px] text-emerald-600 dark:text-emerald-300">{p.recentRating ? `최근 ${p.recentRating.toFixed(1)}` : "평점 —"}</div>
                    </div>
                  </>
                );
        ...
        <article className="mt-8 space-y-4 text-[15px] leading-relaxed text-neutral-700 dark:text-neutral-300">
          ...
          <p>중원은 <strong className="text-neutral-900 dark:text-white">{names(mf)}</strong>로 구성됐다.</p>
          <h3 className="text-lg font-bold text-neutral-900 dark:text-white pt-1">포백 수비진</h3>
          <p>수비진에는 <strong className="text-neutral-900 dark:text-white">{names(df)}</strong>가 자리했다.</p>
          <h3 className="text-lg font-bold text-neutral-900 dark:text-white pt-1">골문</h3>
          <p>골키퍼는 <strong className="text-neutral-900 dark:text-white">{names(gk)}</strong>가 지킨다.</p>
          {kr.length > 0 && (
            <>
              <h3 className="text-lg font-bold text-neutral-900 dark:text-white pt-1">대한민국 주목 포인트</h3>
              <p>{g}조 통합 베스트 11에 대한민국 선수 <strong className="text-neutral-900 dark:text-white">{kr.length}명</strong>({names(kr)})이 포함됐다. 손흥민·이강인·김민재 등 유럽파를 앞세운 한국은 조 2위 이상을 노린다.</p>
            </>
          )}
