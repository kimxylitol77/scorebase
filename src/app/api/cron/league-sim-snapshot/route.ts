// /api/cron/league-sim-snapshot?league=KBO — 시즌 리그 Monte Carlo 일일 스냅샷 (우승·PO 확률 추이).
...
export async function GET(req: Request) {
  ...
  const league = (url.searchParams.get("league") ?? "KBO").toUpperCase();
  if (!SUPPORTED.has(league)) {
    return NextResponse.json({ ok: false, error: `unsupported league ${league}` }, { status: 400 });
  }
  try {
    ...
    if (finished < 20) {
      return NextResponse.json({ ok: true, skipped: `finished ${finished} < 20` });
    }
    ...
    return NextResponse.json({ ok: true, league, date, teams: data.length, top: `${top.team} ${(top.champion * 100).toFixed(1)}%` });
  } catch (e) {
