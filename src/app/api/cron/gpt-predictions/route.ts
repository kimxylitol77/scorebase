...
export async function GET(req: Request) {
  ...
  try {
    const result = await withLlmTag("gpt-predictions", () => runFetchGptPredictions());
    // 회원 커스텀 봇 픽 생성 피기백 — 향후 24h 저장 pred 매치 (실패해도 본 잡 영향 없음)
    const memberBotPicks = await withLlmTag("gpt-predictions", () => runGenerateMemberBotPicks()).catch(() => null);
