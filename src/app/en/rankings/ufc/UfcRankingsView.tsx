// UFC 랭킹 인터랙티브 뷰 (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
"use client";
...
function Avatar({ f, size }: { f: RankedFighter; size: number }) {
  const label = f.name.trim().charAt(0);
  if (f.headshot) {
    ...
    <div className="space-y-5">
          ...
          <div className="min-w-0">
            ...
            {active.champion.href ? (
              <Link href={active.champion.href} className="block truncate text-lg font-black hover:underline">
                {active.champion.name}
              </Link>
            ) : (
              <div className="truncate text-lg font-black">{active.champion.name}</div>
            )}
      ...
      <div className="overflow-hidden rounded-2xl bg-white ring-1 ring-black/5 shadow-[0_24px_70px_-30px_rgba(15,23,30,0.18)] dark:bg-white/[0.04] dark:ring-white/10 dark:shadow-none">
        ...
        {active.ranks.length === 0 && (
          <div className="p-6 text-center text-sm text-neutral-400">Ranking data coming soon</div>
        )}
      </div>

      <p className="text-[11px] text-neutral-400">
        Source: official UFC.com rankings · updated weekly by media panel vote.
      </p>
    </div>
  );
}
