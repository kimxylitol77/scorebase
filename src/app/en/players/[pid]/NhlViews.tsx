// NhlViews (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
...
function TeamCell({ team, info }: { team: string; info?: TeamInfo }) {
  const ko = toEnglishTeamName(team) || team;
  const inner = (
      ...
      <table className="w-full text-sm">
        <thead className="bg-neutral-50 dark:bg-white/[0.04] text-xs text-neutral-500">
          <tr>
            <th className="text-left px-3 py-2 font-medium">Date</th>
            <th className="text-left px-2 py-2 font-medium">Opponent</th>
            <th className="text-right px-2 py-2 font-medium">Result</th>
    ...
    <>
      ...
      {c && c.gamesPlayed != null && (
        <section className="rounded-2xl bg-white p-5 ring-1 ring-black/5 shadow-[0_24px_70px_-30px_rgba(15,23,30,0.18)] dark:bg-white/[0.04] dark:ring-white/10 dark:shadow-none">
          <h2 className="text-xs font-bold uppercase tracking-[0.2em] text-neutral-500 mb-3">Career</h2>
...
export async function NhlPlayerView({ pid }: { pid: string }) {
  ...
  const teamKo = profile.teamFullName ? toEnglishTeamName(profile.teamFullName) || profile.teamFullName : "";
  ...
  return (
    <article className="relative max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div className="space-y-1">
            ...
            <div className="text-[11px] text-neutral-400">
              Official NHL API · {seasonStart}-{String(seasonStart + 1).slice(2)} Season
              {profile.draftYear ? ` · ${profile.draftYear} draft ${profile.draftOverall}Rank` : ""}
            </div>
          </div>
        </div>
      </header>
      ...
      <p className="text-[11px] text-neutral-500 leading-relaxed">
        ⓘ Data source: official NHL API (api-web.nhle.com) · season, career and awards included.
      </p>
    </article>
  );
}
