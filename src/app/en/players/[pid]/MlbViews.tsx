// MlbViews (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
...
function BioLine({
  ...
  const parts = [phys, debut ? `Debut ${debut}` : null, draft || null, school || null].filter(
    Boolean,
  ) as string[];
...
function Footer() {
  return (
    <p className="text-[11px] text-neutral-500 leading-relaxed">
      ⓘ Data sources: official MLB Stats API (statsapi.mlb.com) · Statcast percentiles from Baseball Savant.
      Season and advanced metrics (wRC+, WAR, FIP) refresh after every game.
    </p>
  );
}
...
function HitterGameLog({ games, hrefs }: { games: HitterRecentGame[]; hrefs: Map<string, string> }) {
  if (games.length === 0)
    return <p className="text-sm text-neutral-500">No recent games.</p>;
  return (
...
export async function MlbHitterView({
  ...
  const batsLabel =
    profile.bats === "L" ? "Bats L" : profile.bats === "R" ? "Bats R" : profile.bats === "S" ? "Switch" : "";
    ...
    <>
      ...
      {s && (
        <section className="rounded-2xl bg-white p-5 ring-1 ring-black/5 shadow-[0_24px_70px_-30px_rgba(15,23,30,0.18)] dark:bg-white/[0.04] dark:ring-white/10 dark:shadow-none">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            <StatCard label="AVG" value={s.avg ?? "—"} accent />
            ...
            <StatCard label="G" value={s.games != null ? String(s.games) : "—"} />
          </div>
        </section>
      )}
      <HitterTrendChart rows={career} />
      <Suspense fallback={<ChartSkeleton label="Batted-ball distribution" />}>
      ...
      {c && (
        <section className="rounded-2xl bg-white p-5 ring-1 ring-black/5 shadow-[0_24px_70px_-30px_rgba(15,23,30,0.18)] dark:bg-white/[0.04] dark:ring-white/10 dark:shadow-none">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            <StatCard label="AVG" value={c.avg ?? "—"} accent />
            ...
            <StatCard label="G" value={c.games != null ? String(c.games) : "—"} />
          </div>
        </section>
      )}
    </>
  ...
  return (
    <article className="relative max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8">
      ...
      <PlayerHeader
        ...
        sub={
          <>
            {profile.team ? `${toEnglishTeamName(profile.team)} · ` : ""}
            {profile.birthCity}
      ...
      <BaseballPlayerSeo
        ...
        league="MLB"
        path={`/players/${pid}`}
        team={profile.team ? toEnglishTeamName(profile.team) : null}
        position="Batter"
        ...
        statLine={
          s?.avg
            ? `${season} season AVG ${s.avg}${s.hr != null ? `, ${s.hr}HR` : ""}${s.rbi != null ? ` ${s.rbi}RBI` : ""}.`
            : null
        }
      />
      <PlayerTabs
        tabs={[
          ...
          { key: "seasons", label: "Season log", content: <HitterSeasonTable rows={career} advanced /> },
          { key: "games", label: "G", content: <HitterGameLog games={recent} hrefs={await mlbGameHrefs(recent)} /> },
          { key: "splits", label: "Splits", content: <SplitsView splits={splits} group="hitting" /> },
        ]}
      />
      <Footer />
    </article>
      ...
      <table className="w-full text-sm">
        <thead className="bg-neutral-50 dark:bg-white/[0.04] text-xs text-neutral-500">
          <tr>
            ...
            <th className="text-right px-2 py-2 font-medium">BB</th>
            <th className="text-right px-2 py-2 font-medium">H</th>
            <th className="text-right px-3 py-2 font-medium">Season ERA</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-black/5 dark:divide-white/5">
          {games.map((g) => (
...
export async function MlbPitcherView({
  ...
  const handLabel = profile.hand === "L" ? "LHP" : profile.hand === "R" ? "RHP" : "Switch";
      ...
      {s && (
        <section className="rounded-2xl bg-white p-5 ring-1 ring-black/5 shadow-[0_24px_70px_-30px_rgba(15,23,30,0.18)] dark:bg-white/[0.04] dark:ring-white/10 dark:shadow-none">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            ...
            <StatCard label="SO" value={s.so != null ? String(s.so) : "—"} />
            <StatCard label="BB" value={s.bb != null ? String(s.bb) : "—"} />
            <StatCard label="HR allowed" value={s.hra != null ? String(s.hra) : "—"} />
            <StatCard label="AVG against" value={s.avg ?? "—"} />
          </div>
        </section>
      )}
      <PitcherTrendChart rows={career} />
      {arsenal.length > 0 && <PitchArsenal pitches={arsenal} />}
      <Suspense fallback={<ChartSkeleton label="Velocity by inning" />}>
        <VelocityLoader pid={pid} season={season} />
      </Suspense>
      <Suspense fallback={<ChartSkeleton label="Pitch location" />}>
      ...
      {c && (
        <section className="rounded-2xl bg-white p-5 ring-1 ring-black/5 shadow-[0_24px_70px_-30px_rgba(15,23,30,0.18)] dark:bg-white/[0.04] dark:ring-white/10 dark:shadow-none">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            ...
            />
            <StatCard label="G" value={c.games != null ? String(c.games) : "—"} />
            <StatCard label="Starter" value={c.gs != null ? String(c.gs) : "—"} />
            <StatCard label="IP" value={c.ip ?? "—"} />
            <StatCard label="SO" value={c.so != null ? String(c.so) : "—"} />
          </div>
        </section>
      )}
    </>
  ...
  return (
    <article className="relative max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8">
      ...
      <BaseballPlayerSeo
        ...
        statLine={
          s?.era != null
            ? `${season} season ERA ${s.era}${s.whip != null ? `, WHIP ${s.whip}` : ""}.`
            : null
        }
      />
      <PlayerTabs
        tabs={[
          { key: "overview", label: "Overview", content: overview },
          { key: "seasons", label: "Season log", content: <PitcherSeasonTable rows={career} advanced /> },
          { key: "games", label: "G", content: <PitcherGameLog games={recent} hrefs={await mlbGameHrefs(recent)} /> },
          { key: "splits", label: "Splits", content: <SplitsView splits={splits} group="pitching" /> },
        ]}
      />
      <Footer />
    </article>
