// KboNpbViews (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
...
import BaseballPlayerSeo from "@/components/en/players/BaseballPlayerSeo";
  ...
  return (
    <section className="rounded-2xl bg-white p-5 ring-1 ring-black/5 shadow-[0_24px_70px_-30px_rgba(15,23,30,0.18)] dark:bg-white/[0.04] dark:ring-white/10 dark:shadow-none">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
        ...
        <Stat label="SO" value={c.so != null ? String(c.so) : "—"} />
      </div>
    </section>
  );
}
...
function HitterCareerCard({ c, label }: { c: HitterSeasonRow; label?: string }) {
  return (
    <section className="rounded-2xl bg-white p-5 ring-1 ring-black/5 shadow-[0_24px_70px_-30px_rgba(15,23,30,0.18)] dark:bg-white/[0.04] dark:ring-white/10 dark:shadow-none">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
        <Stat label="AVG" value={c.avg ?? "—"} accent />
...
function Footer({ source }: { source: string }) {
  return (
    <p className="text-[11px] text-neutral-500 leading-relaxed">ⓘ Data source: {source}</p>
  );
}
...
const LEADER_KO: Record<string, string> = {
  BA: "AVG", HR: "HR", RBI: "RBI", WIN: "Wins", SAVE: "SV", K: "K", ERA: "ERA",
};
...
function KboProfileChips({ p }: { p: { salary?: string; signingBonus?: string; draft?: string; debutYear?: string } }) {
  ...
  if (p.salary) items.push(["Salary", p.salary]);
  if (p.signingBonus) items.push(["Signing bonus", p.signingBonus]);
  if (p.draft) items.push(["Drafted", p.draft]);
  if (p.debutYear) items.push(["Joined", p.debutYear]);
  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {rows.map((r) => (
        ...
        >
          {LEADER_KO[r.category]} League {r.rank}
        </span>
      ))}
    </div>
  );
...
function KboRecentGames({ games }: { games: KboPitcherRecentGame[] }) {
  if (games.length === 0)
    return <p className="text-sm text-neutral-500">No recent appearances.</p>;
  return (
      ...
      <table className="w-full text-sm">
        <thead className="bg-neutral-50 dark:bg-white/[0.04] text-xs text-neutral-500">
          <tr>
            ...
            <th className="text-right px-3 py-2 font-medium">Game ERA</th>
          </tr>
        </thead>
...
const KBO_RESULT_KO: Record<string, { label: string; cls: string }> = {
  W: { label: "W", cls: "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300" },
  L: { label: "L", cls: "bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-300" },
  S: { label: "", cls: "bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-300" },
  H: { label: "HLD", cls: "bg-violet-100 text-violet-700 dark:bg-violet-500/20 dark:text-violet-300" },
};
      ...
      <table className="w-full text-sm">
        <thead className="bg-neutral-50 dark:bg-white/[0.04] text-xs text-neutral-500">
          <tr>
            ...
            <th className="text-right px-2 py-2 font-medium">BB</th>
            <th className="text-right px-2 py-2 font-medium">H</th>
            <th className="text-right px-3 py-2 font-medium whitespace-nowrap">Cumulative ERA</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-black/5 dark:divide-white/5">
          {rows.map((g, i) => {
      ...
      <table className="w-full text-sm">
        <thead className="sticky top-0 bg-neutral-50 dark:bg-neutral-900 text-xs text-neutral-500">
          <tr>
            ...
            <th className="text-right px-3 py-2 font-medium whitespace-nowrap">Cumulative AVG</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-black/5 dark:divide-white/5">
          {rows.map((g, i) => (
      ...
      {past.map((b) => (
        ...
        >
          <summary className="cursor-pointer select-none px-4 py-3 text-sm font-semibold text-neutral-600 dark:text-neutral-300">
            {b.season} Game-by-game log{" "}
            <span className="text-xs font-normal text-neutral-400">
              {b.rows.length}G{b.rows[0]?.team ? ` · ${b.rows[0].team}` : ""}
            </span>
          </summary>
...
function kboHandBats(profile: { hand?: "L" | "R"; bats?: "L" | "R" }): string {
  const h = profile.hand === "L" ? "Throws L" : profile.hand === "R" ? "Throws R" : "";
  const b = profile.bats === "L" ? "Bats L" : profile.bats === "R" ? "Bats R" : "";
  return h && b ? `${h}/${b}` : h || b;
}
...
async function KboPitcherView({
  ...
  const name = profile.name ?? "(name unavailable)";
      ...
      {stats && (
        <section className="rounded-2xl bg-white p-5 ring-1 ring-black/5 shadow-[0_24px_70px_-30px_rgba(15,23,30,0.18)] dark:bg-white/[0.04] dark:ring-white/10 dark:shadow-none">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            ...
            <Stat label="SO" value={stats.k != null ? String(stats.k) : "—"} />
            <Stat label="BB" value={stats.bb != null ? String(stats.bb) : "—"} />
            <Stat label="HR allowed" value={stats.hr != null ? String(stats.hr) : "—"} />
            <Stat label="AVG against" value={fmtNum(stats.avg, 3)} />
          </div>
        </section>
      )}
      {percentiles && (
  ...
  return (
    <article className="relative max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8">
      ...
      <Header
        ...
        sub={
          <>
            ...
            Official KBO (koreabaseball.com)
          </>
        }
        extra={
          (profile.birthday || profile.career) && (
            <div className="text-xs text-neutral-500 space-y-0.5">
              {profile.birthday && <div>Date of birth: {profile.birthday}</div>}
              {profile.career && <div>Career: {profile.career}</div>}
            </div>
          )
        }
      />
      <BaseballPlayerSeo
        ...
        statLine={
          stats?.era != null
            ? `${season} season ERA ${stats.era}${stats.wins != null && stats.losses != null ? `, ${stats.wins}W ${stats.losses}L` : ""}.`
            : null
        }
      />
      ...
      />
      <Footer source="Official KBO (koreabaseball.com) · yearly and split records included" />
    </article>
  );
}
...
async function KboHitterView({
  ...
  const percentiles = profile.name
    ...
    : null;
  const season = new Date().getUTCFullYear();
  const batsLabel = profile.bats === "L" ? "Bats L" : profile.bats === "R" ? "Bats R" : "";
  const ops =
  ...
  const overview = (
    <>
      <section className="rounded-2xl bg-white p-5 ring-1 ring-black/5 shadow-[0_24px_70px_-30px_rgba(15,23,30,0.18)] dark:bg-white/[0.04] dark:ring-white/10 dark:shadow-none">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
          <Stat label="AVG" value={stats.avg ?? "—"} accent />
  ...
  return (
    <article className="relative max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8">
      ...
      <BaseballPlayerSeo
        ...
        path={`/players/${pid}?league=KBO`}
        team={team ?? null}
        position={profile.position ? profile.position.replace(/\(.+\)/, "").trim() : "Batter"}
        photo={kboPhotoUrl(pid)}
        height={profile.height ?? null}
        weight={profile.weight ?? null}
        statLine={
          ...
          stats.avg && stats.avg !== "-"
            ? `${season} season AVG ${stats.avg}${stats.hr ? `, ${stats.hr}HR` : ""}${stats.rbi ? ` ${stats.rbi}RBI` : ""}.`
            : null
        }
      />
      <LeaderBadges rows={leaderRanks} />
      ...
      <table className="w-full text-sm">
        <thead className="sticky top-0 bg-neutral-50 dark:bg-neutral-900 text-xs text-neutral-500">
          <tr>
            ...
            <th className="text-right px-2 py-2 font-medium">H</th>
            <th className="text-right px-2 py-2 font-medium">HR</th>
            <th className="text-right px-3 py-2 font-medium whitespace-nowrap">Pitches</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-black/5 dark:divide-white/5">
          {list.map((g) => {
      ...
      {blocks.map((b, i) => (
        ...
        >
          <summary className="cursor-pointer select-none px-4 py-3 text-sm font-semibold text-neutral-600 dark:text-neutral-300">
            {b.season} Game-by-game log{" "}
            <span className="text-xs font-normal text-neutral-400">
              {b.rows.length}G{b.rows[b.rows.length - 1]?.team ? ` · ${b.rows[b.rows.length - 1].team}` : ""}
            </span>
          </summary>
          <div className="px-2 pb-2">
  ...
  if (games.length === 0)
    return (
      <p className="text-sm text-neutral-500">
        No cumulative data yet — it appears one or two days after an outing.
      </p>
    );
  return (
    <div className="rounded-xl bg-white ring-1 ring-black/5 overflow-x-auto dark:bg-white/[0.04] dark:ring-white/10">
      <table className="w-full text-sm">
        <thead className="bg-neutral-50 dark:bg-white/[0.04] text-xs text-neutral-500">
          <tr>
            <th className="text-left px-3 py-2 font-medium">Date</th>
            <th className="text-left px-3 py-2 font-medium">Opponent</th>
            <th className="text-right px-3 py-2 font-medium">Home / away</th>
            <th className="text-right px-3 py-2 font-medium">Score</th>
            <th className="text-right px-3 py-2 font-medium">Team result</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-black/5 dark:divide-white/5">
          {games.map((g, i) => (
...
async function NpbPitcherView({
  ...
  const koName = profile.name ? npbDisplayName(profile.name, profile.kana) : "(name unavailable)";
  ...
  const birthdayKo = profile.birthday
    ? profile.birthday.replace(/年/, " ").replace(/月/, "Month ").replace(/日/, "")
    : undefined;
      ...
      {stats && (
        <section className="rounded-2xl bg-white p-5 ring-1 ring-black/5 shadow-[0_24px_70px_-30px_rgba(15,23,30,0.18)] dark:bg-white/[0.04] dark:ring-white/10 dark:shadow-none">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            ...
            <Stat label="SO" value={stats.k != null ? String(stats.k) : "—"} />
            <Stat label="BB" value={stats.bb != null ? String(stats.bb) : "—"} />
            <Stat label="HR allowed" value={stats.hra != null ? String(stats.hra) : "—"} />
            <Stat label="ER" value={stats.er != null ? String(stats.er) : "—"} />
          </div>
        </section>
      )}
      {percentiles && (
  ...
  return (
    <article className="relative max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8">
        ...
        extra={
          (birthdayKo || profile.name) && (
            <div className="text-xs text-neutral-500 space-y-0.5">
              {profile.name && (
                <div>
                  Japanese name: {profile.name}
                  {profile.kana ? ` (${profile.kana})` : ""}
                </div>
              )}
              {birthdayKo && <div>Date of birth: {birthdayKo}</div>}
            </div>
          )
        }
      />
      ...
      />
      <Footer source="Official NPB (npb.jp) · game logs from the box-score archive" />
    </article>
  );
}
  ...
  const overview = (
    <>
      <section className="rounded-2xl bg-white p-5 ring-1 ring-black/5 shadow-[0_24px_70px_-30px_rgba(15,23,30,0.18)] dark:bg-white/[0.04] dark:ring-white/10 dark:shadow-none">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
          <Stat label="AVG" value={stats.avg != null ? stats.avg.toFixed(3) : "—"} accent />
          <Stat label="OPS" value={ops} accent />
          <Stat label="HR" value={stats.hr != null ? String(stats.hr) : "—"} />
          <Stat label="RBI" value={stats.rbi != null ? String(stats.rbi) : "—"} />
          <Stat label="SB" value={stats.sb != null ? String(stats.sb) : "—"} />
    ...
    <article className="relative max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8">
      ...
      <BaseballPlayerSeo
        ...
        statLine={
          ...
          stats.avg != null
            ? `${stats.season} season AVG ${stats.avg.toFixed(3).replace(/^0/, "")}${stats.hr ? `, ${stats.hr}HR` : ""}${stats.rbi ? ` ${stats.rbi}RBI` : ""}.`
            : null
        }
      />
      <PlayerTabs
