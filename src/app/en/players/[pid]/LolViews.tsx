...
const WINDOW_KO: Record<number, string> = {
  10: "Last 10 games", 20: "Last 20 games", 30: "Last 30 games", 40: "Last 40 games", 50: "Last 50 games",
};
...
function LolCareer({ career, champs, form }: { career: LolCareerLine; champs: LolCareerChamp[]; form: Array<{ window: number; line: LolCareerLine }> }) {
  return (
    <div className="space-y-5">
      <section>
        <h3 className="mb-2 text-sm font-semibold">Career totals</h3>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          <StatBox label="Career record" value={`${career.win}W ${career.lose}L`} sub={`Win rate ${pct(career.winRate)}`} />
          <StatBox label="KDA" value={career.kda.toFixed(2)} sub={`${d1(career.k)} / ${d1(career.d)} / ${d1(career.a)}`} />
          <StatBox label="Kill participation" value={pct(career.part)} />
          <StatBox label="CS per min" value={d1(career.csPerMin)} />
          <StatBox label="Gold per min" value={Math.round(career.goldPerMin).toLocaleString()} />
          <StatBox label="Damage per min" value={Math.round(career.dmgPerMin).toLocaleString()} />
        </div>
      </section>

      {form.length > 0 && (
        <section>
          <h3 className="mb-2 text-sm font-semibold">Recent form</h3>
          <div className="overflow-x-auto rounded-xl bg-white ring-1 ring-black/5 dark:bg-white/[0.04] dark:ring-white/10">
            <table className="w-full text-sm">
              <thead className="bg-neutral-50 text-xs text-neutral-500 dark:bg-white/[0.04]">
                <tr>
                  ...
                  <th className="px-2 py-2 text-right font-medium">Average K/D/A</th>
                  <th className="px-2 py-2 text-right font-medium">KDA</th>
                  <th className="px-3 py-2 text-right font-medium">KP</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-black/5 dark:divide-white/5">
                {form.map((f) => (
      ...
      {champs.length > 0 && (
        <section>
          <h3 className="mb-2 text-sm font-semibold">Career champion pool</h3>
            ...
            {champs.map((c) => (
              <div key={c.id} className="flex items-center gap-2 rounded-xl bg-white px-2.5 py-2 ring-1 ring-black/5 dark:bg-white/[0.04] dark:ring-white/10">
                ...
                <div className="min-w-0 leading-tight">
                  ...
                  <div className="text-[11px] tabular-nums text-neutral-500">{c.played} games · {pct(c.winRate)}</div>
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <article className="relative max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      <header className="space-y-3">
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div className="space-y-1">
            ...
            <div className="text-sm text-neutral-500">{leagueLabel} · {games} sets played</div>
      ...
      <p className="text-[11px] text-neutral-500 leading-relaxed">
        ⓘ Data source: TheSports LoL. The Overview, Games and Champion tabs come from per-set scoreboards we collect, so they cover recent seasons only. The Career tab uses TheSports career totals (record and champion pool since debut, plus last 10-50 game form).
      </p>
    </article>
  );
}
