// BaseballTrendChart (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
"use client";
...
export function HitterTrendChart({ rows }: { rows: HitterSeasonRow[] }) {
  ...
  return (
    <Chart
      data={data}
      title="Slash line by season"
      hint="OBP · SLG · OPS — higher means more production"
      lines={[
...
export function PitcherTrendChart({ rows }: { rows: PitcherSeasonRow[] }) {
  ...
  return (
    <Chart
      data={data}
      title="ERA · FIP by season"
      hint={hasFip ? "Lower is better · FIP strips out luck and defence" : "Lower is better"}
      lines={[
