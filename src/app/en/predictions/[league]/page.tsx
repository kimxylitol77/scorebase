// /en/predictions/[league] — 리그별 시즌 예측 (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
...
import UclBracket from "@/components/en/UclBracket";
...
import { toEnglishTeamName, enLeagueName, LEADER_CATEGORY_EN } from "@/lib/i18n/en";
...
import { type LeaderRow } from "@/components/en/LeagueLeaderBoard";
import { CATEGORIES_BY_LEAGUE, LEAGUE_TO_SPORT } from "@/components/en/leaderboard-categories";
...
import TeamFormBadges from "@/components/en/predictions/TeamFormBadges";
import ValueBetIndicator from "@/components/en/predictions/ValueBetIndicator";
import KeyMatchPreview from "@/components/en/predictions/KeyMatchPreview";
...
function displayTeamName(name: string | undefined | null, league?: string): string {
  ...
  if (/^(TBD|TTBD|TBDT)$/i.test(trimmed)) return "[TBD]";
  const ko = toEnglishTeamName(trimmed);
  // 사전 변환 성공 시 그대로 (Bodø/Glimt → 보되/글림트 등)
  if (ko !== trimmed) return ko;
  // 변환 실패 + slash placeholder 패턴 → 미정 처리
  if (/\//.test(trimmed)) return "[TBD]";
  return trimmed;
}
...
> = {
  ...
  NBA: {
    name: "NBA",
    subtitle: "NBA — season simulation",
  ...
  NHL: {
    name: "NHL",
    subtitle: "NHL — season simulation",
  ...
  MLB: {
    name: "MLB",
    subtitle: "Major League Baseball — season simulation",
  ...
  LALIGA: {
    name: "LaLiga",
    subtitle: "Spanish LaLiga — season simulation",
  ...
  BUNDESLIGA: {
    name: "Bundesliga",
    subtitle: "German Bundesliga — season simulation",
  ...
  SERIE_A: {
    name: "Serie A",
    subtitle: "Italian Serie A — season simulation",
  ...
  LIGUE_1: {
    name: "Ligue 1",
    subtitle: "French Ligue 1 — season simulation",
  ...
  MLS: {
    name: "MLS",
    subtitle: "Major League Soccer — season simulation",
  ...
  UCL: {
    ...
    subtitle: "UEFA Champions League — season simulation",
  ...
  WORLD_CUP: {
    ...
    subtitle: "2026 World Cup — tournament simulation",
  ...
  KBO: {
    name: "KBO",
    subtitle: "KBO League — season simulation",
  ...
  NPB: {
    name: "NPB",
    subtitle: "NPB — season simulation",
  ...
  LOL: {
    name: "LCK",
    subtitle: "LCK — Elo match probabilities",
  ...
  K_LEAGUE_1: {
    name: "K League 1",
    subtitle: "K League 1 — season simulation",
  ...
  K_LEAGUE_2: {
    name: "K League 2",
    subtitle: "K League 2 — season simulation",
  ...
  J1_LEAGUE: {
    name: "J1 League",
    subtitle: "J1 League — season simulation",
    gradient: "from-red-600 via-rose-500 to-pink-500",
    relegationCount: 3,
    showDraw: true,
  },
  J2_LEAGUE: {
    name: "J2 League",
    subtitle: "J2 League — season simulation",
  ...
  AFC_CL: {
    ...
    subtitle: "AFC Champions League Elite — Elo match probabilities",
  ...
  WNBA: {
    name: "WNBA",
    subtitle: "WNBA — season simulation",
  ...
  UEL: {
    name: "Europa League",
    subtitle: "UEFA Europa League — season simulation",
  ...
  UECL: {
    ...
    subtitle: "UEFA Conference League — season simulation",
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const canonical = `/en/predictions/${upper}`;
  if (GROUPED_STANDINGS_LEAGUES.has(upper)) {
    const name = enLeagueName(upper);
    return {
      title: `${name} table`,
      description: `${name} 2026 100-Year Vision League — standings by regional group.`,
  ...
  if (!VALID.includes(upper as ValidLeague)) {
    ...
    if ((ALL_LEAGUES as readonly string[]).includes(upper)) {
      ...
      return {
        title: `${name} table`,
        description: `${name} current season table.`,
        alternates: { canonical },
      };
    }
  ...
  if (upper === "NPB" || upper === "MLB") {
    ...
    const koName = upper === "NPB" ? "NPB" : "MLB";
    const headKw = upper === "NPB" ? "Match analysis" : "Analysis"; // 각 리그의 실제 검색어에 맞춤
    return {
      title: `${upper} ${headKw} (${dateLabel}) — today's ${koName} predictions and win probability`,
      description:
        `Today's ${upper} match analysis (${dateLabel}) — ${koName} match probabilities and starting pitchers, ` +
        `Title probability from an Elo plus market-odds model, refreshed daily. Prediction accuracy is published alongside.`,
      keywords: [
        `${upper} analysis`,
        `${upper} match analysis`,
        `${koName} analysis`,
        `${koName} match analysis`,
        `${upper} prediction`,
        `${upper} predictions`,
        `Today's ${upper}`,
        `${upper} win probability`,
        "baseball analysis",
        "baseball analysis",
        "AI predictions",
        "Scorebase",
  ...
  if (upper === "KBO") {
    ...
    const dateLabel = `${kst.getUTCMonth() + 1}-${kst.getUTCDate()}`;
    return {
      title: `KBO Postseason Probability (${dateLabel}) — Postseason and Title Probability Simulation`,
      description:
        `${dateLabel} KBO postseason (top five) and title probabilities. ` +
        `The remaining fixtures are simulated 5,000 times with an Elo-based Monte Carlo model, refreshed daily — with today's match probabilities and starting pitchers.`,
      keywords: [
        "KBO postseason probability",
        "postseason probability",
        "KBO postseason probability",
        "KBO title probability",
        "Korean baseball title probability",
        "KBO predictions",
        "Korean baseball predictions",
        "KBO season simulation",
        "AI predictions",
        "Scorebase",
      ],
      alternates: {
  ...
  return {
    title: `${info.name} Predictions — Match Probabilities and Title Simulation`,
    description: `${info.subtitle}. Updated daily — ${info.name} match probabilities (Elo plus market odds), Monte Carlo season simulation, and title and play-off probabilities.`,
    keywords: [
      `${info.name} prediction`,
      `Today's ${info.name}`,
      `${info.name} win probability`,
      `${info.name} pick`,
      `${info.name} analysis`,
      "AI predictions",
      "Scorebase",
...
export default async function LeaguePredictions({ params }: Props) {
  ...
  const teamKoNameById = new Map(
    teams.map((t) => [t.id, toEnglishTeamName(t.name)]),
  );
  ...
  if (orphanIds.size > 0) {
    ...
    for (const t of orphanTeams) {
      ...
      teamKoNameById.set(t.id, toEnglishTeamName(t.name));
    }
  }
  ...
  for (const r of leaderRowsRaw) {
    ...
    // 영어판 — 영문명이 없고 원본이 한글이면 낼 이름이 없어 건너뛴다.
    if (!r.playerNameEn && /[가-힣]/.test(r.playerName)) continue;
    const localizedPlayer = r.playerNameEn ?? r.playerName;
    const localizedTeam = toEnglishTeamName(r.teamName);
    leaderRowsByCategory[r.category].push({
      ...
      value: r.value,
      unit: LEADER_CATEGORY_EN[r.category] ?? (/[가-힣]/.test(r.unit ?? "") ? null : r.unit),
  ...
  const buildChampionTrend = (snaps: { date: string; data: unknown }[]): ChampionTrend | null => {
    ...
    const topTeams = [...latest]
      ...
      .slice(0, 6)
      .map((r, i) => ({ en: r.team, name: toEnglishTeamName(r.team), color: TREND_COLORS[i] }));
    return {
    ...
    <div className="relative">
      ...
      <section className="relative overflow-hidden border-b border-neutral-200 dark:border-neutral-800">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
          ...
          <div className="mt-3 flex flex-wrap gap-3 text-xs text-neutral-500">
            <span>Finished {finishedCount} matches</span>
            <span className="text-neutral-300">·</span>
            <span>Upcoming {scheduledCount} matches</span>
            {canSimulate && (
              <>
                <span className="text-neutral-300">·</span>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pb-16 space-y-12">
        ...
        {!canSimulate && (
          <div className="rounded-2xl border border-dashed border-neutral-300 dark:border-neutral-700 p-12 text-center">
            ...
            <p className="mt-2 text-sm text-neutral-500">
              Matches completed so far {finishedCount} — accuracy improves as the season progresses.
            </p>
          </div>
        )}
        ...
        {["KBO", "MLB", "NPB"].includes(upper) && (
          ...
          >
            <div>
              <p className="flex items-center gap-1.5 font-bold break-keep"><CircleDot className="h-4 w-4 shrink-0 text-emerald-500" aria-hidden /> Today's starting pitcher matchups</p>
              <p className="text-xs text-neutral-500 mt-0.5 break-keep">KBO, MLB and NPB starters — ERA, WHIP, K/9 and recent form, updated as line-ups are announced</p>
            </div>
        ...
        {isWorldCup && (
          <section>
            <Heading title="Combined best XI by group" subtitle="The best eleven across each group's four nations · ratings updated daily" />
            ...
            >
              <BookOpen className="h-4 w-4 shrink-0" aria-hidden /> Read the combined best XI analysis →
            </Link>
          </section>
        )}
          ...
          <>
            <section>
              <Heading
                ...
                subtitle="5,000 tournament simulations across 48 nations, seeded by Elo"
              />
              <div className="sm:max-w-xl">
                <MonteCarloBar
                  data={wc
                    ...
                    .slice(0, 14)
                    .map((r) => ({
                      name: toEnglishTeamName(r.teamName),
            ...
            {wcTrend && (
              <section>
                <Heading
                  title="📈 Title probability trend"
                  subtitle="Daily simulation snapshot — how title probability moves with results (top 6)"
                />
            ...
            <section>
              <Heading
                ...
                subtitle="Chance of reaching the final"
              />
              <div className="sm:max-w-xl">
                <MonteCarloBar
                  data={wc
                    ...
                    .slice(0, 14)
                    .map((r) => ({ name: toEnglishTeamName(r.teamName), value: r.final * 100 }))}
                />
              </div>
            </section>
            ...
            <section>
              <Heading
                title="Semi-final probability"
                subtitle="Chance of reaching the semi-finals"
              />
              <div className="sm:max-w-xl">
                <MonteCarloBar
                  data={wc
                    ...
                    .sort((a, b) => b.sf - a.sf)
                    .slice(0, 16)
                    .map((r) => ({ name: toEnglishTeamName(r.teamName), value: r.sf * 100 }))}
                />
              </div>
            </section>
            ...
            <section>
              <Heading
                ...
                subtitle="Top two per group plus the eight best third-placed teams"
              />
              <WorldCupGroupTable
                rows={wc}
        ...
        {isUcl && uclBracketPrevSeason && (
          <details className="rounded-xl border border-neutral-200 dark:border-white/10 px-4 py-3">
            <summary className="cursor-pointer text-sm font-bold text-neutral-600 dark:text-neutral-300">
              Last season ({uclPrevSeasonLabel}) knockout bracket
            </summary>
            <div className="mt-3">
        ...
        {isUcl && !uclBracketPrevSeason && (
          <section>
            <Heading
              title="UCL knockout bracket"
              subtitle="Round of 16 → quarter-finals → semi-finals → final (two legs on aggregate)"
            />
            <UclBracket series={uclBracket} />
          </section>
        )}
        ...
        {isUsPlayoff && playoffBracket.length > 0 && !playoffSeasonDone && (
          <section>
            <Heading
              title={isNhl ? "NHL play-off bracket" : "NBA play-off bracket"}
              subtitle={
                isNhl
                  ? "Round 1 → round 2 → conference finals → Stanley Cup Final (best of 7)"
                  : "Round 1 → conference semis → conference finals → Finals (best of 7)"
              }
            />
            <NbaPlayoffBracket
        ...
        {isUsPlayoff && playoffSeasonDone && (
          <>
            ...
            {lastSeasonPred && (
              <section>
                <Heading
                  title={`${lastSeasonPred.label} season AI prediction record`}
                  subtitle={`Scored ${lastSeasonPred.oneXTwo.evaluated.toLocaleString()} matches measured — the same model carries into next season`}
                />
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[
                    { name: "1X2", stat: lastSeasonPred.oneXTwo },
                    { name: "Over/Under", stat: lastSeasonPred.over },
                    { name: "Handicap", stat: lastSeasonPred.hc },
                    ...
                    >
                      ...
                      <div className="text-[11px] text-neutral-500 mt-0.5">
                        {m.stat.correct.toLocaleString()}/{m.stat.evaluated.toLocaleString()} correct
                      </div>
                    </div>
                  ))}
                </div>
                <p className="mt-2 text-[11px] text-neutral-500">
                  Accuracy across all leagues and periods is on the{" "}
                  <Link
            ...
            <p className="text-sm text-neutral-500">
              Last season's play-off bracket is on the{" "}
              <Link
                href={`/standings/${upper}`}
                className="font-bold text-amber-600 dark:text-amber-400 hover:underline"
              >
                {upper} standings page
              </Link>
              .
            </p>
          </>
        ...
        {isUsPlayoff && playoffWinProbs.length > 0 && (
          <section>
            <Heading
              title={isNhl ? "Stanley Cup probability" : "NBA title probability"}
              subtitle="5,000 Elo-based Monte Carlo runs over the remainder of live series and rounds not yet started"
            />
              ...
              <table className="w-full text-sm">
                <thead className="text-[10px] sm:text-[11px] text-neutral-500 dark:text-neutral-400">
                  <tr className="border-b border-neutral-200 dark:border-neutral-800">
                    ...
                    <th className="text-right px-2 py-2 font-medium hidden sm:table-cell">Conf. finals</th>
                    <th className="text-right px-2 py-2 font-medium">Final</th>
                    <th className="text-right px-3 py-2 font-medium">Champion</th>
                  </tr>
                </thead>
                  ...
                  {playoffWinProbs.filter((p) => p.champion >= 0.001 || p.confFinals >= 0.05).map((p) => (
                    <tr key={p.teamId} className="hover:bg-neutral-50 dark:hover:bg-neutral-900/50 transition">
                      <td className="px-3 py-2 flex items-center gap-2 min-w-0">
                        ...
                        >
                          {p.shortName || toEnglishTeamName(p.teamName)}
                        </Link>
                      </td>
        ...
        {isNba && canSimulate && (
          <div className="rounded-2xl border border-dashed border-neutral-300 dark:border-neutral-700 p-6 text-center text-sm text-neutral-500">
            ⓘ The NBA projection is being reworked.
            {playoffSeasonDone
              ? " the bracket and title probability appear here once the next play-offs begin."
              : " play-off bracket and title probability are shown above."}
          </div>
        )}
          ...
          <>
            ...
            {isBaseballSim && baseballTrend && (
              <section>
                <Heading
                  title="📈 Title probability trend"
                  subtitle="Daily simulation snapshot — how title probability moves through the season (top 6)"
                />
                <div className="sm:max-w-2xl">
            ...
            {isKbo && (
              <section>
                <Heading title="🍂 Postseason (top five) probability" subtitle="Finishing fifth or higher — postseason chance" />
                <div className="sm:max-w-xl">
                  <MonteCarloBar
                    data={mc
            ...
            {info.relegationCount > 0 && (
              <section>
                <Heading
                  title="Top four (UCL) probability"
                  subtitle="Finishing fourth or higher"
                />
                <div className="sm:max-w-xl">
                  <MonteCarloBar
                    data={mc
            ...
            {info.relegationCount > 0 && (
              <section>
                <Heading
                  title={`Relegation probability (bottom ${info.relegationCount} teams)`}
                  subtitle="In the relegation zone at season's end"
                />
                <div className="sm:max-w-xl">
                  <MonteCarloBar
                    data={mc
            ...
            {externalStandings.length > 0 && (
              <section>
                <Heading
                  title="Current table vs Elo projection"
                  subtitle="Current table against our projected finish — big gaps flag likely risers and fallers"
                />
            ...
            <section>
              <Heading
                title="Projected final position"
            ...
            {baseballH2H && (
              <section>
                <Heading
                  title="⚔️ Head-to-head matrix"
                  subtitle="2026 head-to-head W-D-L from the row team's view — deeper green means stronger, updated as matches finish"
                />
                <div className="rounded-xl border border-neutral-200 dark:border-neutral-800 overflow-x-auto">
                  <table className="w-full text-xs min-w-[640px]">
                    <thead>
                      <tr className="bg-neutral-50 dark:bg-neutral-900 text-[11px] text-neutral-500">
                        <th className="px-2 py-2 text-left font-semibold sticky left-0 bg-neutral-50 dark:bg-neutral-900">Team \ Opponent</th>
                        {baseballH2H.teams.map((t) => (
                          <th key={t} className="px-1 py-2 text-center font-semibold">{toEnglishTeamName(t).split(" ")[0]}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
                      {baseballH2H.teams.map((row) => (
                        <tr key={row}>
                          <td className="px-2 py-1.5 font-bold whitespace-nowrap sticky left-0 bg-white dark:bg-neutral-950">{toEnglishTeamName(row).split(" ")[0]}</td>
                  ...
                  <div className="px-3 py-2 text-[11px] text-neutral-400 bg-neutral-50/50 dark:bg-neutral-900/40 border-t border-neutral-200 dark:border-neutral-800">
                    W-D-L from the row team's view · darker means dominance — aggregated across the season
                  </div>
                </div>
              </section>
            )}
        ...
        {upcoming.length > 0 && (
          <section>
            <Heading
              title="Upcoming matches — estimated probabilities"
              subtitle={`Next ${horizonDays} days of scheduled matches`}
            />
              ...
              <table className="w-full text-sm table-fixed">
                ...
                <thead className="bg-neutral-50 dark:bg-neutral-900 text-xs text-neutral-500">
                  <tr>
                    ...
                    <th className="text-right px-2 py-2.5 font-medium">Home</th>
                    <th className="text-center px-1 py-2.5 font-medium"></th>
                    <th className="text-left px-2 py-2.5 font-medium">Away</th>
                    <th className="text-left px-4 py-2.5 font-medium">Win probability (home/draw/away)</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
                  {upcoming.map((m) => {
        ...
        {!isWorldCup && leaderTops.length > 0 && (
          <section>
            <Heading
              ...
              subtitle={`${leaderSeason} season · category leaders`}
            />
        ...
        <div className="text-[11px] text-neutral-500 leading-relaxed">
          ⓘ Statistical estimates from Elo ratings plus home advantage · 5,000 Monte Carlo
          simulations. Actual matches may differ.
        </div>
      </div>
    </div>
  );
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 px-3 py-2 bg-neutral-50 dark:bg-neutral-900 border-t border-neutral-200 dark:border-neutral-800 text-[11px] text-neutral-500">
          {top4Cutoff > 0 && (
            <span className="inline-flex items-center gap-1.5">
              ...
              <span>Top {top4Cutoff} (UCL places)</span>
            </span>
          )}
          {relegationCount > 0 && (
            <span className="inline-flex items-center gap-1.5">
              <span className="inline-block w-3 h-3 rounded bg-rose-500/30 border-l-2 border-rose-500" />
              <span>Relegation zone (bottom {relegationCount} teams)</span>
            </span>
          )}
        </div>
      )}
        ...
        >
          ...
          <ul className="divide-y divide-neutral-200 dark:divide-neutral-800">
            {list.map((r) => {
              ...
              const isKorea =
                /korea/i.test(r.teamName) || r.teamName.includes("Korea");
              return (
                <li
                  key={r.teamName}
                ...
                >
                  ...
                  <span className="text-xs tabular-nums text-neutral-500">
                    {r.expectedPoints.toFixed(1)} pts
                  </span>
                  <span
...
function CurrentVsPredictionCard({
  ...
  return (
    <div className="grid sm:grid-cols-2 gap-4">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-[11px] text-neutral-500 dark:text-neutral-400">
              <th className="px-3 py-1.5 text-left font-semibold">Current</th>
              <th className="px-3 py-1.5 text-left font-semibold">Team</th>
              <th className="px-3 py-1.5 text-center font-semibold">Δ</th>
              <th className="px-3 py-1.5 text-right font-semibold">Prediction</th>
            </tr>
          </thead>
      ...
      <div className="rounded-xl border border-neutral-200 dark:border-white/10 bg-white dark:bg-white/[0.02] overflow-hidden">
        <div className="px-3 py-2 text-xs font-bold border-b border-neutral-200 dark:border-white/10 bg-neutral-50 dark:bg-white/[0.04]">
          Biggest gaps — likely risers and fallers
        </div>
        {movers.length === 0 ? (
          <div className="px-3 py-6 text-xs text-neutral-500 text-center">
            Table and projection agree within 1.5 places — little movement expected
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead>
