// /en/national-teams/[id] — 국가대표팀 상세 (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
...
import GoalHeatmap from "@/components/en/charts/GoalHeatmap";
import type { GoalLineGoal } from "@/components/en/charts/GoalSceneViz";
...
const POS_GROUPS: Array<[string, string]> = [["G", "Goalkeepers"], ["D", "Defenders"], ["M", "Midfielders"], ["F", "Forwards"]];
...
async function wc2026Result(teamIds: number[]): Promise<string | null> {
  ...
  if (finalMatch && last.id === finalMatch.id) return won(last) ? "Winners" : "Runners-up";
  if (wcMatches.length >= 7) return last.homeScore === last.awayScore ? "Semi-finals" : won(last) ? "3rd place" : "4th place";
  if (wcMatches.length === 6) return "Quarter-finals";
  if (wcMatches.length === 5) return "Round of 16";
  if (wcMatches.length === 4) return "Round of 32";
  return "Group stage";
}
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const ko = team ? team.name : "National team";
  ...
  const wcTitleLabel = wcLabel && wcLabel !== "Group stage" ? `2026 World Cup ${wcLabel}` : null;
  const title = [
    `${ko} national football team`,
    [wcTitleLabel, fifaRank ? `FIFA ranking ${fifaRank}` : null].filter(Boolean).join(" · ") || "Squad, manager, fixtures and FIFA ranking",
  ].join(" — ");
  const description =
    `${fifaRank ? `FIFA ranking ${fifaRank} · ` : ""}${ko} national football team (${ko} football team)` +
    `${wcLabel ? ` — 2026 World Cup ${wcLabel}` : ""}. ` +
    `${coachName ? `Manager ${coachName}, ` : ""}Latest call-up squad, recent results and upcoming fixtures.`;
  const keywords = [
    `${ko} football team`,
    `${ko} national football team`,
    `${ko} national team`,
    `${ko} squad`,
    `${ko} national football team squad`, // 빙 실측 "아르헨티나 축구 국가대표팀 명단" 70노출/0클릭 — 정확 토큰

    `${ko} manager`,
    `${ko} FIFA ranking`,
    `${ko} World Cup`,
    ...(wcLabel ? [`${ko} World Cup result`] : []),
...
export default async function NationalTeamPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const tsPlayers = squad.length
    ...
    : [];
  const koName = new Map<string, string | null>();
  ...
  const form = past.slice(0, 6).map((m) => {
    ...
    return gf > ga ? { r: "W", c: "bg-emerald-500" } : gf < ga ? { r: "L", c: "bg-rose-500" } : { r: "D", c: "bg-neutral-400" };
  });
  ...
  const teamJsonLd = {
    ...
    "@type": "SportsTeam",
    name: `${koCountry} national football team`,
  ...
  const breadcrumbJsonLd = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "2026 World Cup", item: `${SITE_URL}/world-cup` },
      { "@type": "ListItem", position: 2, name: "48 qualified nations", item: `${SITE_URL}/national-teams` },
      { "@type": "ListItem", position: 3, name: `${koCountry} national football team`, item: `${SITE_URL}/national-teams/${team.id}` },
    ],
  };
  ...
  return (
    <main className="relative max-w-3xl mx-auto px-4 py-6">
      ...
      <nav className="mb-3 text-xs text-neutral-500 flex items-center gap-1.5">
        <Link href="/en/world-cup" className="inline-flex items-center gap-1 hover:underline" prefetch={false}>
          <Trophy className="h-3.5 w-3.5" aria-hidden /> 2026 World Cup
        </Link>
        <span>›</span>
        <Link href="/en/national-teams" className="hover:underline" prefetch={false}>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.15em] text-white/90 ring-1 ring-white/20">
              <span className="h-1.5 w-1.5 rounded-full bg-white/80" aria-hidden /> National team
            </span>
            ...
            <h1 className="mt-2 text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight break-keep">
              {koCountry}
              <span className="ml-2 align-middle text-base sm:text-lg font-semibold text-white/80">national football team</span>
            </h1>
            <p className="text-white/70 text-sm mt-1">
              {team.name}{fifaRank ? <span className="ml-2 font-semibold text-white/90">FIFA #{fifaRank}</span> : null}
            </p>
          </div>
        </div>
        {coach && (
          <Link
            href={coach.id ? `/en/coaches/${coach.id}` : "#"}
          ...
          >
            ...
            <div className="min-w-0">
              <div className="text-[11px] uppercase tracking-wider text-white/60">Manager</div>
              <div className="font-bold text-lg leading-tight truncate">
                {coach.name}
              </div>
              <div className="text-xs text-white/70">
                {coach.nationality}{coach.age ? ` · ${coach.age}` : ""}{coach.preferredFormation ? ` · prefers ${coach.preferredFormation}` : ""}
      ...
      {form.length > 0 && (
        <section className="mt-6">
          <h2 className="text-sm font-bold text-neutral-500 mb-2">Recent form</h2>
      ...
      {goalSpots.length >= 3 && (
        <section className="mt-6">
          <h2 className="text-sm font-bold text-neutral-500 mb-2"><Goal className="inline h-4 w-4 mr-1 text-rose-500 align-[-2px]" aria-hidden /> Goal location heatmap <span className="text-neutral-400 font-normal">· {goalSpots.length} goals this season</span></h2>
      ...
      {upcoming.length > 0 && (
        <section className="mt-6">
          <h2 className="text-sm font-bold text-neutral-500 mb-2">Next match</h2>
          <div className="space-y-2">
            {upcoming.slice(0, 5).map((m) => (
              <Link key={m.id} href={`/en/scores?sport=soccer&date=${new Date(m.startTime.getTime() + 9 * 3600e3).toISOString().slice(0, 10)}`}
      ...
      {keyPlayers.length > 0 && (
        <section className="mt-6">
          <h2 className="text-sm font-bold text-neutral-500 mb-2">Key players <span className="text-neutral-400 font-normal">· World Cup finals vs friendlies</span></h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {keyPlayers.map(({ p, st }) => {
                ...
                <>
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-[10px] flex items-center gap-x-2 gap-y-0.5 mt-0.5 flex-wrap">
                      {(st.wcG > 0 || st.wcA > 0) && (
                        <span className="text-emerald-600 dark:text-emerald-400 font-semibold">🏆 Finals {st.wcG > 0 ? `⚽${st.wcG}` : ""}{st.wcG > 0 && st.wcA > 0 ? " " : ""}{st.wcA > 0 ? `🅰️${st.wcA}` : ""}</span>
                      )}
                      {(st.frG > 0 || st.frA > 0) && (
                        <span className="text-neutral-500">Friendlies {st.frG > 0 ? `⚽${st.frG}` : ""}{st.frG > 0 && st.frA > 0 ? " " : ""}{st.frA > 0 ? `🅰️${st.frA}` : ""}</span>
                      )}
              ...
              return hasTsp.has(p.id) ? (
                <Link key={p.id} href={`/en/transfers/${p.id}`} className={`${cls} transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] hover:-translate-y-0.5 hover:ring-black/10 dark:hover:ring-white/20 hover:bg-neutral-50 dark:hover:bg-white/[0.04]`}>
                  {inner}
                </Link>
              ) : (
      ...
      {squad.length > 0 ? (
        <section className="mt-6">
          <div className="flex items-baseline justify-between mb-2">
            <h2 className="text-sm font-bold text-neutral-500">Squad <span className="text-neutral-400 font-normal">({squad.length})</span></h2>
            <span className="text-[11px] text-neutral-400">{isOfficialSquad ? "Official call-up list" : "Based on the most recent call-up"}</span>
          </div>
        ...
        <p className="mt-6 text-sm text-neutral-500 text-center py-8">No call-up data yet. This updates as the next match approaches.</p>
      )}

      {/* 조별 순위 — 이 팀이 속한 월드컵 조 (경기·승·무·패·득실·승점) */}
      {groupRows.length > 0 && (
        <section className="mt-6">
          <h2 className="text-sm font-bold text-neutral-500 mb-2">Group {group} table</h2>
          <div className="rounded-xl ring-1 ring-black/5 dark:ring-white/10 overflow-hidden">
            ...
            {groupRows.map((r) => {
              ...
              return tid && !isMe ? (
                <Link key={r.team} href={`/en/national-teams/${tid}`} className={`${cls} transition-colors duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] hover:bg-neutral-50 dark:hover:bg-white/[0.04]`}>{inner}</Link>
              ) : (
                <div key={r.team} className={cls}>{inner}</div>
              );
            })}
          </div>
          <p className="mt-1.5 text-[11px] text-neutral-400">Top two go through to the round of 32 · third-placed teams compete for wildcard spots</p>
        </section>
      )}
    </main>
  );
