// 이적 표시 헬퍼 (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.

/** TheSports transferDesc → 한글 (이적료 없는 건의 유형 표시) */
export const DESC_KO: Record<string, string> = { Free: "Free transfer", Loan: "Loan", "End of loan": "Loan return", "Loan return": "Loan return", Retired: "Retired", Unknown: "" };

/** TheSports 특수 "팀명" (실클럽 아님) → 한글 */
export const SPECIAL_TEAM_KO: Record<string, string> = { "Free player": "Free agent", Retired: "Retired", Disqualification: "Suspension", Unknown: "Undisclosed" };
...
export function badgeOf(t: { fromTeamName: string | null; toTeamName: string | null; transferType: number | null }): string | null {
  ...
  if (t.transferType === 2) return "Return";
  if (t.transferType === 6) return "Released";
  if (t.transferType === 7) return "Free agent";
  return null;
}
