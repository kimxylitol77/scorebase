// 선수 강점·약점 백분위 (영어판). scripts/en-mirror 로 자동 생성.
...
const TRAIT_LABELS: Record<string, string> = {
  ...
  dribbleRate: "Dribble success",
  duelRate: "Duels won",
  tackles: "Tackles",
  interceptions: "Interceptions",
  foulsDrawn: "Fouls won",
  dribbledPast: "Defensive solidity",
};
  ...
  return (
    <section className="rounded-2xl bg-white p-4 sm:p-5 ring-1 ring-black/5 shadow-[0_24px_70px_-30px_rgba(15,23,30,0.18)] dark:bg-white/[0.04] dark:ring-white/10 dark:shadow-none">
      <h2 className="text-base font-bold tracking-tight mb-3">
        <span className="bg-gradient-to-r from-emerald-500 to-rose-500 bg-clip-text text-transparent">Strengths · weaknesses</span>
      </h2>
      <div className="grid sm:grid-cols-2 gap-4">
        {strengths.length > 0 && <Chips title="Strengths" items={strengths} tone="up" suffix={(p) => `top ${100 - p}%`} />}
        {weaknesses.length > 0 && <Chips title="Weaknesses" items={weaknesses} tone="down" suffix={(p) => `bottom ${p}%`} />}
      </div>
      <p className="text-[11px] text-neutral-400 mt-3">Percentile against players in the same position (minimum 450 minutes).</p>
    </section>
  );
}
