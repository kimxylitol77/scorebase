// 선수 경기 로그 표 (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
"use client";
...
const RES_META: Record<string, { ko: string; cls: string }> = {
  W: { ko: "W", cls: "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300" },
  D: { ko: "D", cls: "bg-neutral-200 text-neutral-600 dark:bg-neutral-700 dark:text-neutral-300" },
  L: { ko: "L", cls: "bg-rose-100 text-rose-700 dark:bg-rose-900/40 dark:text-rose-300" },
};
...
function statChips(r: MatchLogRow) {
  ...
  if (shots > 0) {
    out.push({
      ...
      text: (r.shotsOn ?? 0) > 0 ? `Shots ${shots} (on target ${r.shotsOn})` : `Shots ${shots}`,
    });
  }
  if ((r.keyPasses ?? 0) > 0) {
    out.push({ key: "kp", icon: <Compass className="w-3 h-3" aria-hidden />, cls: "text-blue-600 dark:text-blue-400", text: `Key passes ${r.keyPasses}` });
  }
  const def = [
    (r.tackles ?? 0) > 0 ? `Tackles ${r.tackles}` : null,
    (r.interceptions ?? 0) > 0 ? `Interceptions ${r.interceptions}` : null,
  ].filter(Boolean);
  if (def.length) {
  ...
  if ((r.dribbles ?? 0) > 0) {
    out.push({
      ...
      text: (r.dribblesAtt ?? 0) > 0 ? `Dribbles ${r.dribbles}/${r.dribblesAtt}` : `Dribbles ${r.dribbles}`,
    });
  }
  return out;
}
...
function teamLine(name: string, logo: string | null, score: number | null, bold: boolean) {
  return (
    <div className="flex items-center gap-1.5 min-w-0">
      ...
      <span className={`truncate ${bold ? "font-bold" : "text-neutral-500"}`}>{name}</span>
    ...
    <>
      <div className="flex flex-col items-center gap-0.5 w-12 shrink-0">
        ...
        {r.leagueFlag ? (
          ...
          <span className={`text-[9px] font-bold ${r.compKo === "World Cup" ? "text-emerald-600 dark:text-emerald-400" : "text-neutral-400"}`}>{r.compKo}</span>
        ) : null}
      </div>
      ...
      <div className="flex items-center gap-2 shrink-0 ml-auto">
        {played ? (
          ...
          <span className="text-[11px] text-neutral-400">Bench</span>
        )}
    ...
    <section className="rounded-xl bg-white ring-1 ring-black/5 overflow-hidden dark:bg-white/[0.04] dark:ring-white/10">
      ...
      {seasonAgg && seasonAgg.length > 0 && (
        <div className="border-b border-black/5 dark:border-white/5">
          <div className="px-4 pt-3.5 pb-2 flex items-baseline justify-between">
            ...
            <span className="text-[11px] text-neutral-500">club matches only</span>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-[11px] text-neutral-400">
                  ...
                  <th className="px-2 py-1.5 text-right font-medium">Apps</th>
                  <th className="px-2 py-1.5 text-right font-medium hidden sm:table-cell">Starts</th>
                  <th className="px-2 py-1.5 text-right font-medium">Goals</th>
                  <th className="px-2 py-1.5 text-right font-medium">Assists</th>
                  <th className="px-2 py-1.5 text-right font-medium">Rating</th>
                  <th className="px-2 py-1.5 text-right font-medium hidden sm:table-cell">Y/R</th>
                </tr>
              </thead>
              <tbody>
                {seasonAgg.map((a) => {
      ...
      <div className="px-4 pt-3.5 pb-2 flex items-baseline justify-between">
        ...
        <span className="text-[11px] text-neutral-500">
          {filtered.length === rows.length ? `last ${rows.length}Apps` : `${filtered.length} matches · ${rows.length}`}
        </span>
      </div>
      {(seasons.length > 1 || comps.length > 1) && (
        <div className="space-y-1.5 px-4 pb-3">
          {seasons.length > 1 && <FilterRow label="Season" options={seasons} value={season} onChange={setSeason} />}
          {comps.length > 1 && <FilterRow label="Competition" options={comps} value={comp} onChange={setComp} />}
        </div>
      )}
      <div>
        ...
        {rest.length > 0 && (
          <details className="group">
            <summary className="px-4 py-2.5 text-xs text-rose-600 dark:text-rose-400 cursor-pointer select-none list-none marker:hidden hover:underline text-center">
              Previous {rest.length} more matches
            </summary>
            {rest.map((r) => <Row key={r.id} r={r} />)}
          </details>
        )}
        {filtered.length === 0 && (
          <p className="px-4 py-6 text-center text-xs text-neutral-500">No matches match these filters.</p>
        )}
      </div>
    </section>
  );
