// app__teams___id___page (영어판). scripts/en-mirror 로 자동 생성 — 직접 수정하지 말 것.
...
import TeamHistory, { type TeamHistoryData } from "@/components/en/teams/TeamHistory";
import LolTeamRoster from "@/components/en/LolTeamRoster";
...
import TeamSeasonPanel, { type SeasonSlice, type PanelTeamStat, type PanelXgItem } from "@/components/en/teams/TeamSeasonPanel";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const ko = toEnglishTeamName(team.name);
  ...
  if (BASEBALL_LEAGUES.has(team.league)) {
    ...
    try {
      ...
      if (next) {
        ...
        const oppKo = toEnglishTeamName(oppName);
        ...
        const form = w + d + l > 0 ? `Last ${w + d + l} matches ${w}W${d > 0 ? ` ${d}D` : ""} ${l}L` : null;
        title = `${ko} ${intent.replace("Fixtures", "Fixtures")} — next match ${dateLabel} ${oppKo} vs`;
        description =
          `${ko}${enName} next match ${dateLabel} ${oppKo}.${form ? ` ${form}.` : ""} ` +
          `${team.league} ${intent.replace("Fixtures", "Fixtures")} and AI predictions on one page.`;
      }
    } catch {
      // 매치 조회 불가 시 정적 폴백 유지
    }
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      type: "profile",
      images: ogPageImage({ title: ko, subtitle: `${team.league} — Standings, Fixtures, Roster and AI Predictions`, tag: team.league }),
    },
  };
}
...
export default async function TeamPage({ params }: Props) {
  ...
  // 관련 기사는 한국어 콘텐츠라 영어판에서는 비운다 (아래 articles 사용처가 자동으로 숨는다)
  const [dbMatches, recentMatches, upcoming, articlesRaw, tsTeamRows] = await Promise.all([
  ...
  const articles: typeof articlesRaw = [];
    ...
    for (const m of [...upcoming, ...recentMatches]) {
      ...
      try {
        ...
        recentLineup = {
          ...
          oppKo: toEnglishTeamName(opp.name),
  ...
  if (recentLineup) {
    ...
    for (const r of koRows) {
      koMap.set(r.name, null); // 영어판 — 한글명을 쓰지 않는다
    }
    lineupPlayers = recentLineup.xi.map((n) => ({
      name: n,
      ko: koMap.get(n) ?? (n !== n ? n || null : null),
    }));
  }
  ...
  const koName = (id: number, en: string) =>
    playerNameResolved.get(id)?.ko ?? en;
  ...
  const coach =
    ...
    (bbCoach
      ...
      : null);
  const coachRole = coach && !coach.id && bbCoach?.interim ? "Caretaker" : "Manager";
  ...
  const sportLabel =
    ({ soccer: "Football", baseball: "Baseball", basketball: "Basketball", hockey: "Ice Hockey", esports: "Esports" } as Record<string, string>)[sport] ?? "Football";
  ...
  if (tsTeamRows.length) {
    ...
    squad = members
      .map((m) => {
        ...
        return { id: m.id, name: t?.name || "Player", photo: T_PHOTOS[m.id] || t?.photoUrl || null, pos: squadPos(m.id, m.coarse ?? t?.position), value: valueOf.get(m.id) ?? 0, number: m.number, flag: ov?.flag || null };
      })
      // 이름없음("선수") 제외 + 동일 표시명 중복 제거
      .filter((p) => { if (p.name === "Player" || seen.has(p.name)) return false; seen.add(p.name); return true; })
      // 시장가치순, 동가면 등번호순
  ...
  if (sport === "soccer") {
    ...
    for (const m of withStats) {
      ...
      xgAll.push({
        ...
        opp: toEnglishTeamName(isHome ? m.awayTeam.name : m.homeTeam.name),
  ...
  return (
    <div className="relative">
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: jsonLdScript({
            ...
            "@type": "SportsTeam",
            name: toEnglishTeamName(team.name),
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                ...
                {clubRank && (
                  <Link
                    ...
                    title="World club ranking"
                  >
                    <Globe className="h-3 w-3" aria-hidden /> World {clubRank}
                  </Link>
                )}
              </div>
              <span className="inline-flex items-center gap-1.5 rounded-full bg-rose-500/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.15em] text-rose-600 ring-1 ring-rose-500/20 dark:text-rose-400">
                <span className="h-1.5 w-1.5 rounded-full bg-rose-500" aria-hidden /> Team profile
              </span>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-relaxed text-neutral-600 dark:text-neutral-300 break-keep">
                {toEnglishTeamName(team.name)} {teamIntentKeywords(team.league)}  — current standing, next fixture, roster and AI predictions, kept up to date.
              </p>
              {coach && (() => {
                const inner = (
                  <>
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[11px] text-neutral-500 truncate">
                        {[coach.nationality, coach.age ? `${coach.age}` : null, coach.preferredFormation ? `prefers ${coach.preferredFormation}` : null].filter(Boolean).join(" · ")}
                        {coach.id && <span className="text-rose-600 dark:text-rose-400"> · profile →</span>}
                      </div>
                    </div>
                  </>
                );
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 space-y-10">
        ...
        <TeamAbout
          name={toEnglishTeamName(team.name)}
          enName={toEnglishTeamName(team.name) === team.name ? "" : team.name}
          leagueLabel={enLeagueName(team.league)}
        ...
        {T_HISTORY[String(teamId)] && (
          <TeamHistory data={T_HISTORY[String(teamId)]} teamName={toEnglishTeamName(team.name)} />
        )}
        ...
        {teamVenue && (teamVenue.venueName || teamVenue.foundation || teamVenue.totalPlayers) && (
          <section>
            <SectionH title="Club info" icon={<Landmark className="h-5 w-5" aria-hidden />} />
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {teamVenue.venueName && (
                <Stat label="Stadium" value={venueKo(teamVenue.venueName)!} subtle={[teamVenue.capacity ? `${teamVenue.capacity.toLocaleString()} capacity` : "", cityKo(teamVenue.city) ?? ""].filter(Boolean).join(" · ")} />
              )}
              {teamVenue.foundation ? <Stat label="Founded" value={`${teamVenue.foundation}`} /> : null}
              {teamVenue.totalPlayers ? (
                <Stat label="Squad" value={`${teamVenue.totalPlayers}`} subtle={teamVenue.foreignPlayers != null ? `foreign ${teamVenue.foreignPlayers}` : undefined} />
              ) : null}
              {squadValueM != null ? (
                <Stat label="Squad value" value={`€${squadValueM}M`} />
              ) : null}
            </div>
            {teamVenue.website && (
        ...
        <section className="grid sm:grid-cols-3 gap-4">
          <Card title="Last 5 form">
            {recentForm.results.length > 0 ? (
              <div className="space-y-2">
                <FormDots results={recentForm.results} />
                <div className="text-xs text-neutral-500 tabular-nums">
                  {recentForm.wins}W {recentForm.draws}D {recentForm.losses}L
                </div>
              </div>
            ) : (
          ...
          <Card title="Home / away split">
            <div className="space-y-1.5 text-sm">
              <div>
                🏠 <span className="font-semibold">{ha.home.wins}W {ha.home.draws}D {ha.home.losses}L</span>
                <span className="text-xs text-neutral-500 ml-2">per match {ha.home.ppg.toFixed(2)} pts</span>
              </div>
              <div>
                ✈ <span className="font-semibold">{ha.away.wins}W {ha.away.draws}D {ha.away.losses}L</span>
                <span className="text-xs text-neutral-500 ml-2">per match {ha.away.ppg.toFixed(2)} pts</span>
              </div>
            </div>
          </Card>
        </section>
        ...
        {squad.length > 0 && (
          <section>
            <SectionH title="Squad" subtitle={`By market value · ${squad.length} players · tap for detail`} icon={<Users className="h-5 w-5" aria-hidden />} />
        ...
        {nhlRoster.length > 0 && (
          <section>
            <SectionH title="🏒 Roster" subtitle={`${nhlRoster.length} players · NHL official · tap for detail`} />
            {([["F", "Forwards"], ["D", "Defencemen"], ["G", "Goalies"]] as const).map(([g, label]) => {
        ...
        {mlbRoster.length > 0 && (
          <section>
            <SectionH title="⚾ Roster" subtitle={`${mlbRoster.length} players · MLB official · tap for detail`} />
            {([["P", "Pitchers"], ["C", "Catchers"], ["IF", "Infielders"], ["OF", "Outfielders"]] as const).map(([g, label]) => {
        ...
        {baseballRoster.length > 0 && (
          <section>
            <SectionH title="⚾ Roster" subtitle={`${baseballRoster.length} players · ${team.league} official · tap for detail`} />
            {([["P", "Pitchers"], ["B", "Position players"]] as const).map(([g, label]) => {
        ...
        {nbaRoster.length > 0 && (
          <section>
            <SectionH title="🏀 Roster" subtitle={`${nbaRoster.length} players · ESPN · tap for detail`} />
            {([["G", "Guards"], ["F", "Forwards"], ["C", "Centers"]] as const).map(([g, label]) => {
        ...
        {(INJURY_LEAGUES.has(team.league) || keyPlayers.length > 0) && (
          <section className="grid md:grid-cols-2 gap-5">
            {INJURY_LEAGUES.has(team.league) && (
              <div>
                <SectionH title="Injuries and absences" icon={<HeartPulse className="h-5 w-5" aria-hidden />} />
                <Link
                ...
                >
                  <span className="min-w-0">
                    <span className="block text-sm font-semibold">{toEnglishTeamName(team.name)} View injuries and absences</span>
                    <span className="block text-xs text-neutral-500 mt-0.5">See the full league-wide list by club</span>
                  </span>
            ...
            {keyPlayers.length > 0 && (
              <div>
                <SectionH
                  title="Key players"
                  subtitle="Season goals and assists — top"
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-neutral-50 dark:bg-white/[0.03] text-xs text-neutral-500">
                      <tr>
                        ...
                        <th className="px-2 py-2 font-medium"><Goal className="ml-auto h-4 w-4" aria-label="Goals" /></th>
                        <th className="px-2 py-2 font-medium"><Target className="ml-auto h-4 w-4" aria-label="Assists" /></th>
                        <th className="text-right px-4 py-2 font-medium">Apps</th>
                      </tr>
                    </thead>
        ...
        {recentLineup && lineupPlayers.length === 11 && (
          <section>
            <SectionH
              ...
              subtitle={`${recentLineup.dateLabel} vs ${recentLineup.oppKo}${recentLineup.formation ? ` · ${recentLineup.formation}` : ""}${recentLineup.finished ? "" : " · announced before kick-off"}`}
        ...
        {upcoming.length > 0 && (
          <section>
            <SectionH title="Upcoming matches" subtitle="Next 5" />
            <div className="rounded-xl border border-neutral-200 dark:border-neutral-800 overflow-hidden">
              <table className="w-full text-sm">
                <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
                  {upcoming.map((m) => {
                      ...
                      <tr key={m.id} className="transition-colors duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] hover:bg-neutral-50 dark:hover:bg-white/[0.04]">
                        ...
                        <td className="px-2 py-3 font-medium">
                          ...
                          >
                            ...
                            <span className="truncate">{toEnglishTeamName(opp.name)}</span>
                          </Link>
                        </td>
        ...
        {recentMatches.length > 0 && (
          <section>
            <SectionH title="Recent matches" subtitle="Last 5 results" />
            <div className="rounded-xl border border-neutral-200 dark:border-neutral-800 overflow-hidden">
              <table className="w-full text-sm">
                <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
                  {recentMatches.map((m) => {
        ...
        {articles.length > 0 && (
          <section>
            <SectionH
              ...
              subtitle={`${toEnglishTeamName(team.name)}Recent articles mentioning this club`}
            />
...
function StreakDisplay({
  ...
  let main: { tone: "good" | "bad" | "neutral"; text: string } = {
    tone: "neutral",
    text: "No notable streak",
  };
  if (streak.winningRun >= 2) {
    main = { tone: "good", text: `${streak.winningRun} game win streak` };
  } else if (streak.unbeatenRun >= 3) {
    main = { tone: "good", text: `${streak.unbeatenRun} unbeaten` };
  } else if (streak.losingRun >= 2) {
    main = { tone: "bad", text: `${streak.losingRun} game losing streak` };
  }
  const tone =
    main.tone === "good"
  ...
  return (
    <div>
      ...
      <div className="mt-1 text-xs text-neutral-500 space-y-0.5">
        <div>Clean sheets in the last 5 {streak.cleanSheetsLast5}</div>
        <div>No goals in the last 5 {streak.failedToScoreLast5}</div>
      </div>
    </div>
  );
}
