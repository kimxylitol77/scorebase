// 게시판 글 수정 (관리자 전용) — admin_session 없으면 글로 되돌림. 모든 글 수정 가능.
import { cookies } from "next/headers";
import { notFound, redirect } from "next/navigation";
import Link from "next/link";
import { prisma } from "@/lib/db";
import { COOKIE_NAME as ADMIN_COOKIE, readSessionCookie } from "@/lib/auth";
import { updatePostAdminAction } from "../../actions";
...
export default async function AdminEditPostPage({ params }: Props) {
  ...
  if (!admin) redirect(`/analysis/${postId}`);

  const post = await prisma.post.findUnique({
    where: { id: postId },
    select: { id: true, title: true, content: true, lineupCode: true, author: { select: { nickname: true } } },
  });
  if (!post) notFound();
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      <div className="mb-5 flex items-center justify-between">
        <h1 className="text-lg font-bold">글 수정 (관리자)</h1>
        <span className="text-xs text-neutral-500">작성자: {post.author.nickname}</span>
      </div>
      <form action={updatePostAdminAction} className="space-y-4">
