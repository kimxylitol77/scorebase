    ...
    <main className="max-w-3xl mx-auto px-4 sm:px-6 py-12 sm:py-14">
      <header className="flex items-end justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">스포츠 분석</h1>
        </div>
        <Link
          href="/analysis/new"
          className="inline-flex items-center gap-1.5 rounded-full bg-rose-600 hover:bg-rose-700 text-white px-4 py-2 text-sm font-semibold transition"
        >
          ✏️ 글쓰기
        </Link>
      </header>
            ...
            {posts.map((p) => {
              ...
              return (
                <li key={p.id}>
                  <Link
                    href={`/analysis/${p.id}`}
                    className="grid grid-cols-[1fr] sm:grid-cols-[56px_1fr_150px_72px_56px_56px] gap-2 px-4 py-3 items-center hover:bg-neutral-50 dark:hover:bg-neutral-900/50 transition"
                  >
                    <span className="hidden sm:inline text-[11px] font-bold text-blue-600 dark:text-blue-400">
                      분석
                    </span>
                    <span className="min-w-0">
                      <span className="flex items-center gap-1.5">
                        ...
                        {p.likes > 0 && (
                          <span className="shrink-0 text-[11px] font-semibold text-rose-500">
                            [{p.likes}]
                          </span>
                        )}
                      </span>
                      {/* mobile meta */}
                      <span className="sm:hidden mt-1 flex items-center gap-2 text-[11px] text-neutral-500">
                        <span>
                    ...
                    <span
                      className="hidden sm:flex items-center gap-1 text-xs text-neutral-600 dark:text-neutral-400 min-w-0"
                      title={g.name}
                    >
                      <span>{g.emoji}</span>
                      <span className="truncate">{p.author.nickname}</span>
                    </span>
                    <span className="hidden sm:block text-right text-xs text-neutral-500">
                      {listTime(p.createdAt)}
                    </span>
                    <span className="hidden sm:block text-right text-xs text-neutral-500">
                      {p.views}
                    </span>
                    <span className="hidden sm:block text-right text-xs font-semibold text-rose-500">
                      {p.likes}
                    </span>
                  </Link>
                </li>
