...
## 남은 7개 처리 결과 (2026-08-21)

전수 스캔에서 나온 "컵인데 VALID_LEAGUES 없는 13개" 중 슈퍼컵 6개를 뺀 나머지 7개를
실데이터로 확인하고 셋으로 갈랐다.

| 리그 | 매치 | 순위캐시 | getFullStandings | 판정 |
|---|---|---|---|---|
| UEFA_WCL | 153 | 18행 | **18행 정상** | 처리 — 일정 탭 추가(순위 유지) |
| LEAGUES_CUP | 22 (예정 4) | 36행 | **36행 정상** | 처리 — 일정 탭 추가(순위 유지) + NO_TABLE 오분류 제거 |
| CANADA_CHAMP | 1 | 없음 | — | 처리 — 일정 탭 |
| CLUB_FRIENDLY | 1658 | 없음 | — | **유지** — 문서화된 "스코어 전용" 결정 |
| INTL_FRIENDLY | 403 | 없음 | — | **유지** — 위와 동일 |
| AFCON | **0** | 48행 | **0행** | **별건** — 팀 매핑 0 |
| CONCACAF_GOLD | **0** | 16행 | **0행** | **별건** — 팀 매핑 0 |

**CLUB_FRIENDLY·INTL_FRIENDLY 를 안 건드린 근거.** 메모리 `aiscore-competitor-club-friendly`
에 "스코어 전용 축구 리그 온보딩 레시피 — 기사·예측·순위 제외" 가 명시돼 있다. /scores 에만
노출하는 게 설계 의도다. 추가로 CUP_LEAGUES 에 넣으면 `buildCupBracket` 이 페이지 진입마다
1658건을 homeTeam/awayTeam join 으로 읽는다 — "컵은 매치 수가 적어 부담이 작다"는 전제가
깨진다.

**AFCON·CONCACAF_GOLD 를 안 건드린 근거.** 순위 캐시는 있는데 우리 Team row 가 0이라
매핑이 안 돼 표가 안 그려진다. 페이지 목록에 넣어도 빈 일정 탭이 하나 늘 뿐이다.
`create-missing-standings-teams.ts` 를 돌려봤더니 **"매핑률 0% — 미온보딩 대회로 보고 건너뜀"**
으로 기존 안전선이 스스로 걸러냈다. 국가대표 대회 온보딩이 선행돼야 하는 별건이다.

**cupViews 에 순위 탭을 조건부로 추가했다.** CUP_LEAGUES 는 원래 "1라운드부터 녹아웃" 을
전제했는데 LEAGUES_CUP(A~D조)·UEFA_WCL(리그페이즈)은 조별리그가 있다. 그대로 넣었으면
일정 탭을 얻는 대신 **이미 잘 나오던 표를 잃었다**(1단계만 적용했을 때 실렌더로 확인).
판정은 `NO_TABLE_LEAGUES` 단일 정의를 따르게 했다. FA컵·DFL 슈퍼컵 등 녹아웃 컵은
회귀 없음(실렌더 확인).
