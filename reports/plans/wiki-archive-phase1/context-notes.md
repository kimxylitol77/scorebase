...
## 2단계 진행 로그 (2026-08-09)

- 사용자 지시. 팀 페이지 "시즌 통계·팀 시즌 통계·xG 추이"를 시즌으로 접고 26-27 을 쌓는 식으로, 따로 페이지 만들지 말 것.
- 실측. 시즌 통계·xG 는 Match 에서 재계산 가능(영구 보존), 유일한 소멸 소스는 data/team-season-stats.json(빅4 76팀, 25-26 풀시즌 동결 — 빌더 스크립트는 커밋된 적 없음).
- ts 엔드포인트 복원. `/v1/football/season/recent/team/stat`(단수 stat) — uuid=시즌. 프록시 경유 Vercel 호출 가능. 컵대회 10개는 405(ts 팀집계 미제공).
- `TeamSeasonStatArchive` 신설(teamId+seasonLabel). 백필 76팀(25-26) + 잡 첫 실행 1,099팀(진행 시즌 — K리그 등 비빅4 리그 신규 확보). archive-standings cron 이 함께 실행(maxDuration 300, 실측 2분 9초).
- 페이지. TeamSeasonPanel(클라이언트, 시즌 칩) — 현재 시즌 라이브 계산, **과거 시즌 순위는 SeasonStandingsArchive 정본**(수집 개시 이전 부분 시즌을 매치로 재계산하면 틀림 — 예: K리그 2025 는 6월부터만 수집). xG 는 시즌 창 분할(시즌 전체 평균 + 최근 10 차트).
- 검증. 아스널: 26-27 "쌓이는 중" + 25-26 접힘(1위·85점·38경기 집계·37경기 xG). FC서울: 2026 진행(Elo 포함) + 2025 완료 칩 + ts 통계 신규 표시. KIA: 야구 회귀 없음(단일 슬라이스). 콘솔 에러 0. 테스트 96 통과.
