...
# 2단계 잔여 마무리 (2026-08-09 밤)

- [x] 시즌 라벨 통일 — WALES_PL·MONTENEGRO_1L·LUXEMBOURG_ND 를 SPLIT_YEAR 등록(아카이브 실측 근거: "2025" 행이 32·36·30경기 풀시즌 + 8월 개막 1~2경기). 아카이브 두 테이블 relabel 완료(2025→2025-26 3행, 2026→2026-27 순위 3행·팀통계 42행). 자동 감지는 월드컵 휴지기 오염으로 폐기
- [x] 역사 탭 "시즌별 득점왕·도움왕" 섹션 — LeagueLeader rank1 (EPL 25-26 홀란·브루노 렌더 확인, 내일 백필로 5시즌 확장)
- [x] 팀 페이지 "최근 경기" 행에도 상대전적 링크
- 관찰: 웨일스 아카이브 24행 = af 가 정규+챔피언십 라운드 두 단계를 함께 줌 — 팀 수 표기(/24팀)가 실제(12팀)보다 큼. 마이너 리그 한정 코스메틱, 필요 시 단계 dedup

---

# 3단계 심화 (2026-08-09 밤, "3단계도 진행")

- [x] 배구 순위 아카이브 편입 — VNL·VNL_W·AVC_NATIONS_W·EGL_W (fetchVolleyballTable 재사용, 세트 스코어=득실 칸). VNL 2026 1위 일본 실측
- [x] 축구 커버 확대 — SOCCER_LEAGUES 밖이어도 ts 캐시가 있으면 아카이브(J3 실측 계기). 캐시 폴러가 축구·야구·배구뿐이라 비축구 명시 제외 방식
- [x] 과거 시즌 심화 — MLB·NHL·KBO·NPB 각 +5시즌(2016~2020) = 각 11시즌 보유. 스팟체크 4건 실역사 일치(두산 93승·워싱턴 55승·보스턴·히로시마)
- [ ] NPB 경기별 선수 로그 — 칩 발행(KBO 패턴 재사용)
- 관찰: J3 는 과도기 시즌(2~6월) 표가 온보딩 전 캐시 롤오버로 소멸 — 새 시즌 개막 시 자동 축적 시작. NBA 2021-22 이전은 api-sports 플랜 밖(불변)

---

# 감독 축 점검 (2026-08-09 밤, "감독도 체크해줘")

- [x] 새는 곳 2개 발견·수리
  - careers·honors 빌더가 전체 갱신 덮어쓰기 → 경질 감독의 경력·우승기록이 JSON 에서 소멸(프로필 페이지 죽음) → merge 보존으로 교정
