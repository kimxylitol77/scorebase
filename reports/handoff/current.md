...
## 빅5 주간 리뷰 자동 발행 신설 (8/25, `8b1b653`)

화 11:00 KST 리그별 ANALYSIS — 결과 서사·주간 MVP 선수·**MVP 감독(신규)**·이변.
첫 발행 4건(#4167~#4173, 분데스는 8/28 개막이라 다음 주 합류). 상세는 메모리
`soccer-weekly-review` 와 `reports/plans/big5-weekly-review/`.
핵심 = MVP 감독은 시장 기대 승점 대비 초과성과(개막주 EPL 은 헐 시티 야키로비치),
MVP 선수는 weekly-xi 재사용이라 **산식을 바꾸면 두 글을 같이** 바꿔야 한다.

## 사비뉴 LB 오표시 = ts↔af 오매핑 (8/26, `b6fb1d8`)

빌더의 스탯 지문 매칭이 이름을 안 봐 사비뉴↔오라일리가 붙었다(포지션·생일·출전로그 58행
오염). 전수 스캔으로 진짜 오염 3건 확정·교정, recordMatch 에 이름 게이트(공통 접두 4자 —
브라질 애칭은 startsWith 불성립) 추가. 마린은 진짜 af id 미확인이라 매핑 제거만(공백>오염).
오염은 5곳(맵·포지션 JSON 2종·출전로그·**스쿼드 등번호**·바이오 캐시) — 전부 청소.
배포 검증: 생일 2004-04-10·선호 RW·독립 LB 0건. 등번호는 ISR 만료 후 소거(백그라운드 확인 중),
사비뉴 토트넘 등번호는 af 스쿼드 반영 후 다음 수집이 자동 적재. 상세 = 메모리 `ts-af-player-name-search-fallback`.

### 전수 결과 — 3건이 아니라 242건 (8/26~27, `d6ba5f7`)

af 스쿼드 이름 재검사로 K리그 ~190·라리가 18 등 **242건 오염 제거** + 출전로그 2,555행·
스쿼드 173행 삭제. 재빌드로 매핑 5,445 회복(게이트 실전 거부 57건).

- **게이트 커버리지가 2차 함정** — 이름 유니버스가 몸값 선수만 커버해 K리그가 판정 불가로
  통과, 게이트 있는데도 재빌드가 58건 재오염(실측). 유니버스 확장으로 해결.
- **눈 판정 필수 2부류** — 한국 로마자 음절(편집거리 무의미)·등록 별명(Pepelu 등 12건 유지,
  스윕에 항상 나오는 게 정상).
- 커밋 충돌: player-season-stats.json 은 origin(봇) 채택 — 일일 재생성 파일.
- push 루프 안에서 rebase 충돌 시 자동 abort 되므로, 충돌은 루프 밖에서 수동 해소 후
  `GIT_EDITOR=true git rebase --continue` (에디터 대기로 timeout 나는 함정).

### 잔여 리그 확장 (8/27, `9b5d906`)

빌더 리그 12개의 af 스쿼드 이름을 추가 수집(4,857→10,844)해 미검사 2,024 매핑까지 커버.
오염 17건 추가 제거(지첸코↔Konadu·황인범↔Targhalline 등) — **검사 가능 전역 오염 0.**
유지 목록은 16건으로 확정(별명 12 + 등록명 4) — 스윕에 항상 나오는 게 정상.
지첸코·황인범 등 제거된 선수의 af 데이터는 월요일 주간 리빌드가 올바르게 재매칭한다.

## 배구 토너먼트 대진표 자동 유도 (8/27, `87764b6`)

VB_ASIAN_W 가 8강인데 조별 표만 보이던 것. ts 배구엔 라운드 원천이 없어(stage 미승인)
`knockout-derive.ts` 가 매치 위상으로 8강·준결승·3위전·결승을 유도 — 렌더마다 돌아
**새 배구 토너먼트는 작업 0**. 감시 `vb_bracket_gap` + 스킬 `scorebase-tournament-bracket`.
라벨은 1라운드 팀 수로 — 끝에서 세면 결승이 8강으로 둔갑(미래 라운드 미수집).

+ 라이브 라인업 감독명 → /coaches 링크 (`9dec110`, 프로덕션 무리뉴·마타라초 확인).

### 대진표 v2 + 전 종목 (8/27, `69e8731`)

배구 7개 대회 전수에서 함정 3개 수정 — 순위 결정전 분리(NORCECA)·역산 라벨(U17 24강→16강)·
다단계 혼재 가드(EURO_W 엉터리 대진표 소거). 3·4위전은 결승 식별 후에만 판정.
