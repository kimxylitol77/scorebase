...
async function ollamaReview(prompt, system) {
  const strictSystem = system + " 엄수: diff 내용을 요약·나열·설명하지 마라. 발견한 404/회귀 위험만 최대 3줄, 없으면 '이상 없음' 한 줄만.";
  // qwen 은 system 지시를 약하게 따라 diff 뒤(user 끝)에 형식을 한 번 더 강제한다.
  const strictUser = prompt + "\n\n[형식 엄수] 변경 요약·설명을 절대 쓰지 마라. 404/런타임/회귀 위험만 '파일 — 이유' 한 줄씩 최대 3개. 위험 없으면 정확히 '이상 없음' 만 써라.";
  try {
    const r = await fetch(`${OLLAMA_HOST}/api/chat`, {
      ...
      body: JSON.stringify({
        model: OLLAMA_MODEL,
        messages: [{ role: "system", content: strictSystem }, { role: "user", content: strictUser }],
        stream: false,
