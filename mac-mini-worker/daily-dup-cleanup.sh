...
# 매시 :30 KST — ts 이중저장 Match 중복(SAFE=참조0)만 자동 삭제.
...
# 타이머가 매시로 바뀐 뒤로는 "월요일" 만 보면 하루 24번 돈다 — KST 10시 회차로 못 박는다.
# TZ 를 명시하는 이유. 이 호스트(Vultr)는 UTC 라 그냥 date 를 쓰면 요일·시가 KST 와 어긋난다.
if [[ $(TZ=Asia/Seoul date +%u) == 1 && $(TZ=Asia/Seoul date +%H) == 10 ]]; then
