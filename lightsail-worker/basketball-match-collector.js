...
axios.defaults.headers.common["User-Agent"] = "scorebase-monitor/1.0 (basketball-match-collector)";
const fs = require("fs");
...
const COMP_TO_LEAGUE = {
  ...
  "vl7oqd4t30q510j": "ASIAN_GAMES_BK_W", // 아시안게임 여자농구 (2026-09-21) — diary 실 competition_id 확인(9/18~22 12경기·11개국)
};
...
async function postUnmapped(unmapped) {
  await axios.post(
    ...
    { sport: "basketball", matches: [], unmapped: unmapped.slice(0, 300) },
...
async function poll() {
  ...
  for (const offset of SWEEP) {
    ...
    for (const m of raw) {
      ...
      seen.add(m.id);
      if (m.status_id === 0 || m.status_id === 15) continue; // 0=Abnormal(hide)·15=TBD(시간 미정) skip
