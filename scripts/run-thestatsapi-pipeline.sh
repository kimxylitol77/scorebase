...
#   사용: THESTATSAPI_KEY=... bash scripts/run-thestatsapi-pipeline.sh [상위N=50]
set -e
set -o pipefail
cd "$(dirname "$0")/.."
N="${1:-50}"
...
echo "▶ 1/3 선수 매핑 발굴 (상위 $N)"
npx tsx --env-file=.env.local scripts/discover-thestatsapi-players.ts "$N"
echo "▶ 2/3 시즌 활동 카드"
