...
# 전 단계 멱등이라 반복 실행 안전. 분당 60회 페이싱 내장(한도 120), 2·3단계는 해당 리그 매핑만. 시즌 중 데일리 봇으로 그대로 사용 가능.
...
echo "▶ 2/3 시즌 활동 카드"
npx tsx --env-file=.env.local scripts/build-player-season-heatmaps.ts "--league=$LEAGUE"
echo "▶ 3/3 경기별 히트맵"
npx tsx --env-file=.env.local scripts/build-player-match-heatmaps.ts "--league=$LEAGUE"
echo "PIPELINE_DONE"
