// NPB TheSports player id → 한국어 이름 사전 (Haiku 음역).
// 흐름:
...
async function collectPlayerIds(): Promise<Set<string>> {
  ...
  console.log(`▶ NPB ${matches.length} 매치 detailLive.players 수집`);
...
async function haikuTranslate(
  ...
  // 따로 id ↔ en 매핑 후 결과 적용.
  const prompt =
    `다음 NPB (일본 프로야구) 영문 선수 이름을 한국 스포츠 미디어 표기로 변환해주세요.\n` +
    `참고: 네이버 NPB 페이지의 한국어 표기 기준.\n` +
    `- 일본 선수 (대다수): Sato Teruaki → 사토 테루아키, Murakami Munetaka → 무라카미 무네타카, Yamamoto Yoshinobu → 야마모토 요시노부\n` +
    `- 표기 가이드: 성+이름 사이 띄어쓰기 1개 (사토 테루아키, 오타니 쇼헤이)\n` +
    `- 외국인 용병 (간혹): Aderlin Rodriguez → 아데를린 로드리게스 같은 음역\n` +
    `- 일본어 음역은 외래어 표기법 (는,은,사,자,쟈/자 → 자 / 츠,쯔 → 쓰)\n` +
    `- 자신없는 선수는 결과에서 제외 (잘못된 음역보다 누락이 나음)\n\n` +
    `선수 list:\n` +
    batch.map((b, i) => `${i + 1}. "${b.en}"`).join("\n") +
    `\n\n출력 — JSON 객체 한 줄 (다른 설명 X):\n` +
    `{"Sato Teruaki": "사토 테루아키", "Yamamoto Yoshinobu": "야마모토 요시노부", ...}`;
  ...
  for (const { id, en } of records) {
    ...
    try {
      await prisma.theSportsPlayer.upsert({
        ...
        create: { id, name: en, nameKo: ko, sport: "NPB" },
      });
      upserted++;
    } catch (e) {
