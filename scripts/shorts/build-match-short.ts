...
async function main() {
  ...
  const prompt = `아래 축구 경기 데이터만 근거로 한국어 인사이트 문장 2개를 써라. 규칙: 각 문장 35자 이내(짧게), 아래 브리핑에 있는 숫자만 사용(새 숫자·비율 계산 금지), 팀명은 브리핑 표기 그대로, 이모지·따옴표 금지, 두 문장을 줄바꿈으로만 구분해 출력.\n첫 문장 = 스코어와 xG 의 관계(누가 기회를 더 만들었고 결과는 어땠나). 둘째 문장 = 골 장면이나 최다 슛 선수 중 가장 눈에 띄는 사실.\n\n${brief}`;
  let insight: string[] = [];
  for (let attempt = 0; attempt < 2 && insight.length === 0; attempt++) {
    ...
    lines = lines.filter((l) => l.length >= 8 && l.length <= 64).slice(0, 2);
