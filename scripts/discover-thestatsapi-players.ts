...
// 검색은 부분 문자열 매칭이라 성(마지막 토큰)으로 조회 후 전체 이름 정규화 일치로 확정.
// 이름 불일치·동명 다수는 매핑하지 않고 로그만 남긴다 (틀린 매핑 < 누락).
...
async function main() {
  ...
  const rows = mv
    .map((r) => ({ id: r.id, name: names.get(r.id) ?? "" }))
  ...
  for (const r of rows) {
    ...
    const cands = (res?.data ?? []).filter((p) => norm(p.name) === norm(r.name));
    const c = cands[0];
    const team = c?.current_team;
    if (cands.length !== 1 || !team) {
      console.log(`  ✗ ${r.name} — 후보 ${cands.length}${cands.length > 1 ? " (동명 다수)" : ""}`);
      skipped++;
      continue;
    }
