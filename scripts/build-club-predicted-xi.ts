// 클럽 리그 예상 라인업 빌드 → data/club-predicted-xi.json (5대 리그 + K리그)
// build-wc-predicted-xi 의 클럽 일반화 — 팀별 최근 확정 XI(ts 캐시, 친선·컵 포함) 가중투표로
// 예상 선발 11명 + 선수별 신뢰도 산출. 가중치 = 최근 감쇠 × 대회 계수(자기 리그 1 / 컵 0.75 / 친선 0.5),
// 최신 InjurySnapshot 의 부상 선수는 후보에서 제외해 백업이 자동 승격된다 (Flashscore 공개 방법론 이식).
// 라인업 발표(킥오프 ~1h 전) 전까지 매치 페이지 예정 상태에 노출 — 확정 도착 시 자동 교체.
//
// cron-wc-xi.sh (매일 07:00, 맥북) 에서 실행 — DB 만 읽으므로 ts 화이트리스트 무관.
// 결정 근거: docs/club-predicted-xi/context-notes.md
import { PrismaClient } from "@prisma/client";
import * as fs from "fs";
import { CLUB_XI_LEAGUES, teamNameMatches } from "../src/lib/predict/club-xi-leagues";

const prisma = new PrismaClient();

/** 대상 리그 — 라이브 페이지 노출 게이트와 단일 정의 공유. */
...
interface TeamOut { teamName: string; formation: string; basedOnGames: number; updatedAt: string; xi: PredictedPlayer[] }
...
async function main() {
  const out: Record<string, Record<string, TeamOut>> = {};

  // 현재 스쿼드 (data/team-squads.json, ts team id 키) — 직전 시즌 선발의 이적·방출 필터.
  // 라인업 유래 로스터만으론 프리시즌 미출전 주전(바르사 8/16 실측 5/11 겹침)이 걸러져 보강.
  let squadByTsTeam: Record<string, { squad?: { id: string }[] }> = {};
  try {
    squadByTsTeam = JSON.parse(fs.readFileSync("data/team-squads.json", "utf-8"));
  } catch {
    console.log("team-squads.json 없음 — 최근 라인업 로스터만으로 게이트");
  }
  ...
  for (const league of TARGET_LEAGUES) {
    ...
    if (teamIds.length === 0) {
      console.log(`${league}: 향후 14일 예정 매치 없음 — skip`);
      continue;
    }

    // 최신 부상 스냅샷 (3일 이내) — 팀별 제외 대상. 부상 있는 팀만 row 인 게 정상.
    // ⚠️ 스냅샷 teamId 는 전부 null (수집기 미채움, 8/16 실측) — teamName 정규화 매칭으로 간다.
    ...
    // 우리 teamId → ts team id (스쿼드 파일 키) + 팀명
    ...
    const leagueOut: Record<string, TeamOut> = {};
    ...
    for (const teamId of teamIds) {
      ...
      // 직전 시즌 리그 경기를 섞을 때 이적·방출 선수를 걸러내는 게이트.
      ...
      // (8/16 바르셀로나 실측: XI 에 유스 4명). 직전 시즌 같은 리그 최근 3경기를 낮은 가중으로
      // 섞되, 최근 로스터에 없는 선수(이적·방출)는 투표에서 제외.
      const hasCurrentLeagueGame = xis.some((x) => x.matchLeague === league);
      // 잔류 판정 = 현재 스쿼드 파일 ∪ 최근 라인업 로스터 (스쿼드 파일이 8월 이적을 못 담는
      // 지연·팀 누락 대비 합집합).
      const tsTeamId = tsIdByTeam.get(teamId);
      const currentSquad = new Set(
        (tsTeamId ? squadByTsTeam[tsTeamId]?.squad ?? [] : []).map((s) => s.id),
      );
      const stillAtClub = (id: string) => rosterRecent.has(id) || currentSquad.has(id);
      let prevXis: { players: LuPlayer[]; formation?: string }[] = [];
      ...
      if (recentCount + prevXis.length < 2) continue; // 재료 2경기 미만이면 예측 스킵 (품질 가드)

      // 가중 투표 — 최근 감쇠 × 대회 계수. 직전 시즌 리그 경기는 고정 0.8 (친선 0.5×감쇠보단 높고
      // 이번 시즌 리그 경기(1.0×감쇠 상위)보단 낮게 — 리그 개막 후 자동으로 밀려난다).
      const RECENCY = [3, 2.4, 1.9, 1.5, 1.2];
      const compW = (l: string) => (l === league ? 1 : l === "CLUB_FRIENDLY" ? 0.5 : 0.75);
      // 이 팀의 부상자 — 스냅샷 teamName 정규화 매칭 (teamId 전부 null)
      const ourTeamName = teamName || nameByTeam.get(teamId) || "";
      const teamSnaps = ourTeamName
        ? leagueSnaps.filter((s) => teamNameMatches(s.teamName, ourTeamName))
        : [];
      ...
      if (xi.length === 11) {
        leagueOut[String(teamId)] = {
          teamName: teamName || nameByTeam.get(teamId) || "",
    ...
    out[league] = leagueOut;
    console.log(`${league}: 대상 ${teamIds.length}팀 중 ${Object.keys(leagueOut).length}팀 예상 XI 산출`);
  }

  // 한글명 일괄 주입
  const allIds = new Set<string>();
  for (const lg of Object.values(out)) for (const t of Object.values(lg)) for (const p of t.xi) allIds.add(p.id);
  ...
  for (const lg of Object.values(out)) for (const t of Object.values(lg)) for (const p of t.xi) {
    const ko = koById.get(p.id);
    if (ko) p.nameKo = ko;
  }

  fs.writeFileSync("data/club-predicted-xi.json", JSON.stringify(out, null, 1) + "\n");
  const total = Object.values(out).reduce((s, lg) => s + Object.keys(lg).length, 0);
  console.log(`data/club-predicted-xi.json 저장 — ${total}팀`);
}
