...
## 후속 2차 — 분데스·UCL·MLS 백필
- 스크립트 시즌 계산을 getApiFootballSeason 으로 교체 (MLS 달력연도 분기 — 자체 seasonOf 는 3~6월 매치를 전년 시즌으로 오매핑).
- 알리아스 추가: olympiakos→olympiacos, prague→praha, lafc→angeles (LA Galaxy 는 la·galaxy 토큰이라 충돌 없음).
- 결과: 분데스 306/306(100%)·MLS 391/394(99.2%) 게이트 통과. UCL 188/212(88.7%) 미달 —
  잔여 24건 중 23건이 2026-27 예선(7월, 마이너 클럽)으로 af xG 미제공 영역. UCL 은 게이트대로 미노출이 정답.
- 버그 수정: xG 표 팀 조회가 본 순위표 teamIds 만 써서 MLS(ts 표=컨퍼런스 1개)에서 13팀만 렌더 →
  xG 표 teamIds 합집합으로 수정, 30팀 확인.

## 세션 로그
- 2026-07-18: 문서 생성. Phase 1~3 + 후속(EPL·분데스·UCL·MLS 백필, 매치 상세 부합도, MLS teamMap 수정) 완료·배포.
