...
### 왜 배당 알림이 한 건도 안 왔나 (사용자 제보 "배당은 아직도 수정 안 된 것 같다")

코드 버그가 아니다. `TelegramAlertLog` 전체 기간에 ODDS 종류가 0건인 이유는 **대상 교집합이 비어서**다.

- 배당 옵트인 회원 2명 — 한 명은 즐겨찾기 0개, 다른 한 명은 맨유·첼시(EPL)
- EPL 은 8월 개막이라 예정 경기가 2027년 5월까지 잡혀 있고 프리매치 배당 스냅샷이 **0건**
- 실제로 배당이 움직인 경기(최근 24h 8%+ 5건)는 WNBA·아르헨티나·MLB·COPA_SUD — 아무도 즐겨찾기하지 않은 리그

그래서 "즐겨찾기 한정"이라는 설계 자체가 비시즌에 알림을 0으로 만든다. 이걸 메우는 축이
`ODDS_ALL`(전 경기 급변 다이제스트)이다. 즐겨찾기 한정 알림은 그대로 두고 **선택지로 추가**했다 —
전 경기를 기본으로 켜면 관심 없는 리그로 폭주하기 때문.

### 결정

1. **기존 4종(킥오프·골·종료·팔로우 픽)은 기본 ON.** 이미 받고 있던 회원이 이번 변경으로 갑자기
   못 받으면 그게 회귀다. 새로 여는 것(`alertOddsAll`)만 기본 OFF.
2. **라인업도 기본 ON.** 새 종류지만 즐겨찾기 경기 한정이라 양이 적고, 선발 발표는 관심도가 높다.
3. **골 알림을 끈 회원도 `GOAL:h-a` 상태는 계속 기록한다.** 기록을 멈추면 나중에 켰을 때
   그 경기의 지난 골이 한꺼번에 발송된다(중간 참여자 catch-up 스팸과 같은 함정).
4. **전 경기 다이제스트는 하루 1회 묶음(KST 21시).** 급변마다 개별 발송하면 하루 수십 통이라
   텔레그램 차단 위험이 크다. 봇 다이제스트(`BOT_DIGEST`)와 같은 이유·같은 패턴.
5. **설정 필드는 API 화이트리스트(`ALERT_FIELDS`)로만 수정 가능.** PUT 본문을 그대로 넘기면
   User 의 다른 컬럼까지 열린다.

### 라인업 알림의 구조적 한계 (실측)

최근 14일 라인업 보유 729경기의 `lineupUpdatedAt` 을 킥오프와 비교한 결과.

```
킥오프 30~60분 전 178경기   ← 알림 발송됨
킥오프  0~30분 전 150경기   ← 발송됨
킥오프 직후~30분  120경기   ← 대상 아님(status 가 이미 LIVE)
킥오프 60~90분 후 218경기   ← 대상 아님 (최다)
```

킥오프 **전**에 도착하는 건 372/729 = 51%뿐이다. 나머지는 TheSports 워커가 라이브 폴링 중에
받아 저장하기 때문에 이미 경기가 시작된 뒤다. 즉 라인업 알림은 창을 아무리 넓혀도 절반쯤만 나간다.
이건 발송 로직이 아니라 **수집 시점** 문제라, 개선하려면 예정 경기 라인업을 선제적으로 폴링해야 한다.

### 검증 (2026-07-31)

- DDL 프로덕션 적용 — 장기 트랜잭션 0건 확인 후 `lock_timeout 3s`. 기존 회원 3명 모두
  4종 ON + 라인업 ON + 전 경기 OFF 로 채워졌고, 기존 배당 옵트인 값도 보존됨
- 드라이런(발송 없음) — LINEUP 대상 0건(지금 3시간 내 축구 8경기에 라인업 미도착),
  ODDS_ALL 다이제스트는 지금 만들면 2경기가 담김(아르헨티나 ▲16%·WNBA ▼9%)
- tsc 0 · ESLint 0 · 프로덕션 빌드 통과
- 디스패처 전체 실행은 실제 발송이 나가므로 하지 않음 — 종전 방침 유지
