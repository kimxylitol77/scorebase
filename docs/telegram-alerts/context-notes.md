...
- e2e 검증 — 경기 ⭐ → 마이페이지 방문(동기화) → 킥오프 35분 전 알림 수신.
- **동기화 트리거가 마이페이지 방문뿐**이라는 게 이 설계의 약점이다. 별표만 하고 마이페이지를
  안 들르면 서버에 안 올라간다. 별표 시점에 바로 PUT 하는 쪽이 맞지만, 비로그인 사용자가 다수라
  매 토글마다 401 요청을 쏘게 된다. 사용량을 보고 판단할 것.
