...
- **컨텍스트 맹점 → 해소(2026-07-19 2차)**: PM 이 P2 로 올린 "라이브 한국어 반응 스트림"은 이미 07-18 배포된 티커였음. build_mission 을 3섹션으로 확장 — ① watch(기존 경쟁사 변화, idea-log.jsonl) ② **scout(GPT 신규 발굴, competitor-scout-ideas.jsonl) 연결** ③ **이미 구현된 것(최근 2주 git log 제목 30건) + 재제안 금지 규칙**. 미션 ~6.4k자. 다음 월요 회의부터 적용.
