...
오늘·내일 선발 61명 중 1명(MLB Hayden Wesneski)이 이름·사진만 있고 시즌 성적이 통째로 비어 있었다.
소스(MLB Stats API)에 시즌 등판 기록이 없으면 이렇게 온다.

→ `hasStats()` 로 판정해 개인 카드(페이지·OG·게시판 프리필)에서 빈 타일 4개 대신
**"시즌 기록 미집계 — 리그 공식 기록이 들어오면 자동으로 채워집니다"** 를 띄운다.
"등판 기록 없음" 같이 사실을 단정하지 않는다 — 우리 소스에 값이 없다는 것만 말한다.
매치업 카드는 좌우 비교표라 기존대로 "—" 를 그대로 둔다.
