...
- **원시 좌표 0.46 접기 폐기 → 라인 군집화+균등 배치.** 최전방끼리 하프라인 충돌·라인 간격 4~7% 문제.
  라인 y 균등(GK 92↔최전방 56, 원정 미러, 하프라인 완충 12%), 라인 내 x 균등, 최대 5라인(초과 시
  최근접 인접 라인 병합 — 6라인이면 간격 7.2%로 같은 x 이름표가 아랫라인 원에 겹침, 월드컵 스페인 실측).
- **OG 카드 versus 모드 축소**: 사진 88→68px·이름 22→18px·포지션 라벨 생략(compact = s.side 존재).
  단일 모드는 불변. 기존 사용자 versus 보드에도 적용되는 개선.
