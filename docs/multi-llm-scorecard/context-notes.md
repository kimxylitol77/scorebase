...
### 야구 선발 게이트 후속 — 예측 cron 17:30 추가 (같은 날 발견)

게이트 배포 직후 실측에서 **유실이 실제로 발생하는 조건**이 드러났다. 배포 전 시뮬레이션은 "12h 내 킥오프인데 선발 미공개 = 0건"으로 통과했으나, 측정 시점이 우연히 선발 수집 직후여서 문제가 가려졌다. **한 시점 스냅샷으로 타이밍 문제를 검증하면 안 된다** — cron 시각표와 데이터 도착 시각을 직접 대조할 것.

- `baseball-starters` 는 하루 7회(10:30·11:00·11:30·12:00·12:30·14:00·**17:00** KST)인데, **KBO 선발은 17:00 회차에서야 채워진다**(실측: 오늘 18:30 경기의 homeStarter 최종수정 17:01). 오전 6회는 헛방.
- 반면 `gpt-predictions` 는 13:30·01:30 이었다. 13:30 엔 선발이 없어 게이트가 대기로 거르고, 다음 회차 01:30 엔 경기가 이미 끝나 있다 → **KBO·NPB 픽이 매일 통째로 유실**되는 구조였다. 배포 당일 픽이 나간 건 수동 트리거 때문.
- 해결: `vercel.json` 의 gpt-predictions 를 `30 4,16` → **`30 4,8,16`**(13:30·**17:30**·01:30 KST). 선발 수집 17:00 직후에 붙인다. MLB 는 다음날 07:40~ 경기라 약 14h 여유, KBO 18:30·NPB 18:00 은 30분~1시간 전으로 빠듯하지만 유실은 막는다. 소스가 17시에야 공개하므로 이보다 앞당길 수단은 없다.
- **관찰**: KBO·NPB 픽이 매일 쌓이는지. 안 쌓이면 17:30 회차가 선발 수집을 앞질렀거나 17:00 수집이 실패한 것 — `야구 선발대기 N` 로그와 Match.homeStarter 갱신 시각을 대조할 것.
