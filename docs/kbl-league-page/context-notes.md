...
## 2026-09-20 선수 명단 탭
- 요청 "국내리그 선수페이지 만들자 농구 배구 남자 여자" — 선수 상세는 이미 있으므로 "리그별 선수 목록 페이지"로 해석. 별도 라우트(/players/kbl 등)는 /players/[pid] 동적 세그먼트와 충돌 소지가 있고, 직전 요청("일정은 종목에 순위표랑 같이")과 같은 원칙으로 리그 페이지 탭에 붙였다.
- 팀 묶음 기준은 선수 사전의 teamId → prisma Team(로고·이름). 팀명은 toKoreanTeamName(team-names 사전) 우선, 없으면 nameKo. 팀 순서는 한글명 가나다.
- 위치별 소제목은 팀 페이지 로스터에 이미 있어 여기선 생략(팀당 4열 그리드, 메타 줄에 포지션). 사전 로스터 함수가 이미 포지션→등번호 정렬이라 순서는 유지된다.
- 배지: KBL 은 country≠대한민국(국적명), KOVO 는 foreign 플래그("외국인"). WKBL 사전엔 국적 필드가 없어 배지 없음.
- 갱신일 표시는 KBL 만 export 가 있어 세 리그 통일을 위해 넣지 않았다. 사전은 mac-mini weekly-static-refresh ⑪-c/d/e 가 갱신.
