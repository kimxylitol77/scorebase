...
## 2026-08-01 OU 게이트 재검증 — 일괄 차단 기각, 실측 통과제로 교체
- 계기: /predictions/scorecard 에 "모델 빠진 경기" 다수 문의 → 원인 추적 중 OU 미발행율
  LLM 80~100% 확인 → 규칙 ③ 근거 재검증.
- out-of-sample (게이트 가동 07-19 이후 채점 1,305건 — 원 백테스트 06-27~07-17 과 안 겹침):
  gpt-5.6 58.5%(n142) > scorebase 54.5%(209) = kimi 54.5%(209) > qwen 54.3%(162)
  > grok 53.1%(213) > gemini 52.9%(157) > claude 50.7%(213).
  **"타 모델 44~48% 코인토스 이하" 주장이 전 모델에서 기각** — 원 백테스트는 소표본 노이즈
  (평균 회귀). 미발행 픽도 채점하는 published 설계가 이 재검증을 가능하게 함 (설계 의도 적중).
- 교체: 규칙 ③ = 그림자 실측 통과제. scorebase 앵커 항상 발행, 타 모델은 최근 60일 창
  실측 n>=120 && acc>=50% 일 때만 발행 (publish-gate.ts OU_SHADOW_* 상수).
  실측 미제공·조회 실패·표본 부족 → 기존 일괄 차단과 동일(fail-closed). 고정 명단이 낡아
  생긴 문제라 명단을 갱신하는 대신 데이터 주도로 전환 — 모델이 썩으면 창이 굴러가며 자동 재차단.
- 로더: fetch-gpt-predictions.ts loadOuShadowStats() (10분 캐시, OU 시장에서만 조회).
  gatedUpsert 한 곳에서 주입 → storeAnchor/storePanel/backfill/qwen 맥미니 전 경로 커버.
- 배포 시점 판정 (60일 창, 08-01): claude·gpt-5.6·grok·kimi 발행 개방 / gemini 49.1%·
  qwen 48.5% 차단 유지 — 둘 다 07-19 이후 폼은 50% 위라 옛 표본이 빠지면 자동 개방 전망.
- 단위 테스트 publish-gate.test.ts 10케이스 — 규칙 ①② 보존 + ③ 경계값·fail-closed 고정.

## 2026-08-01 좌석 회전 + 좌석별 성패 로그 (같은 커밋)
- gpt-5.6 이 07-21~25 닷새간 0건인데 아무도 몰랐다(사후 표 대조로 발견). 패널 루프가
  고정 순서라 킥오프 가드·시간 예산의 잘림이 항상 뒤 좌석에 몰리는 구조 문제도 겸함.
- 매치 index 로 시작 좌석 회전(잘림 균등 분배) + 좌석별 ok/fail 카운터를 완료 로그와
  cron 응답(seats)에 노출 — ok=0 & fail>0 좌석은 당일 로그에서 바로 식별.
