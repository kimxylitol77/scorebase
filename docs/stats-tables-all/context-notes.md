...
## 2026-09-24 추가 — 모바일 선수 열 폭

- 신고 "모바일에서 이름 칸이 너무 크고 스탯이 잘 안 보인다". 실측 375px 화면에서 고정(sticky) 선수 열이 198px, 스탯 열 30px. 열 폭이 내용(긴 팀명 보조 텍스트)에 끌려 커진 것.
- 수정. 선수 th/td 에 `w-[8.5rem] max-w-[8.5rem] sm:w-auto sm:max-w-none`, 스탯 th 에 `min-w-[2.75rem]`, 사진 28px(sm 32px). 결과 선수 136px·스탯 44px, 한 화면에 스탯 4열(이전 2열). 이름은 truncate 로 잘리지만 링크·보조 텍스트는 유지.
