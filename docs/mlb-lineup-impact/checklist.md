# 체크리스트
- [x] 산식 모듈 lineup-impact.ts — wOBA·PA(slot)·xR·Δ·수축, 단위 테스트(리그 평균 라인업 → xR = lgR/G, 한 명 교체 시 Δ 부호)
- [x] 박스스코어 파서 — MlbBoxBatter 시즌 성분(pa·ab·h·2b·3b·hr·bb·ibb·hbp·sf) + 벤치 타자 목록(가산 필드)
- [x] 리그 상수 fetch(teams/stats hitting) unstable_cache 1일 + 실패 시 고정값 폴백(4.48 / .312)
